...
PNTM_INTERFACE::PNTM_INTERFACE():IF_serverSock(SERVER, IF_Data.hostName, IF_Data.inPortNum),
...
PNTM_INTERFACE::read(void* thisFunc, void* sock, void* msgQ){
    ...
    while(true){
        ...
        if(msgQNumMsgs(*IF_MsgQID)<IF_Data.packetNum)
            ...
            do{
                msgQReceive(*IF_MsgQID, buff, IF_Data.packetSize, WAIT_FOREVER);
...
int
PNTM_INTERFACE::stateRead(){
    int flag = E.CTRL_STOP_START;
    semTake(Mutex, NO_WAIT);
    //flag = stateFlag;
    flag = E.CTRL_RUN_START;
    semGive(Mutex);
    return flag;
}
