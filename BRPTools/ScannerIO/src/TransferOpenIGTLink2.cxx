...
TransferOpenIGTLink2::TransferOpenIGTLink2()
{
  this->Hostname = "";
  this->Port = -1;
}

int TransferOpenIGTLink2::Init()
{
}

int TransferOpenIGTLink2::Connect()
{
  this->CheckAndConnect();
  return 1;
}

int TransferOpenIGTLink2::Disconnect()
{
  //mutex->acquire();  //lock
  if (connected)
    {
...
void TransferOpenIGTLink2::SetServer(std::string hostname, int port)
{
  this->Hostname = hostname;
  this->Port = port;
}

bool TransferOpenIGTLink2::CheckAndConnect()
{
  if (this->Port > 0)
    {
...
void* TransferOpenIGTLink2::CallReceiveProcess(void*ptr)
{
  TransferOpenIGTLink2* pRT = reinterpret_cast<TransferOpenIGTLink2*>(ptr);
...
void TransferOpenIGTLink2::ReceiveProcess()
{
  int retval;
...
void TransferOpenIGTLink2::Process()
{
  ...
  while (1)
    {
      ...
      std::cerr << "TransferOpenIGTLink2::Process(): Triggered." << std::endl;
      if (this->AcquisitionThread)
        {
