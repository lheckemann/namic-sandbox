...
You will have to place itkAnalyzeObjectLabelMapImageIO.h, itkAnalyzeObjectLabelMapImageIO.cxx, itkAnalyzeObjectLabelMapImageIOFactory.h, itkAnalyzeObjectLabelMapImageIOFactory.cxx into the source of ITK-$>$Code-$>$IO.  While in the source of ITK-$>$Code-$>$IO you will want to change the cmake file so that SET{...} has itkAnalyzeObjectLabelMapImageIOFactory.cxx and itkAnalyzeObjectLabelMapImageIO.cxx in it.  Then you will need to modify itkImageIOFactory.cxx, you will want place \#inlcude "itkAnalyzeObjectLabelMapImageIOFactory.h", with the includes, then add in the function RegisterBuiltInFactories the following code: ObjectFactoryBase::RegisterFactory( AnalyzeObjectLabelMapImageIOFactory::New() );

Then you will have to place the folder called AnalyzeObjectMap into the source of ITK-$>$Utilities.  You will also have to modfify the CMakeLists to include SUBDIRS(AnalyzeObjectMap)
\begin{itemize}
...
This is set to false right now because the reader can not stream data.  This is something that will be worked on in the future.
...
\begin{itemize}
        ...
        This function is the assignment operator, which copies a new object entry from the right side to the AnalyzeObjectEntry variable on the left side.
