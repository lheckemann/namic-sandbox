...
=========================================================================*/
#ifndef _itkScaleVersor3DTransform_txx
#define _itkScaleVersor3DTransform_txx
...
template<class TScalarType>
ScaleVersor3DTransform<TScalarType>::
  ScaleVersor3DTransform( unsigned int spaceDimension,
  ...
  ::ComputeMatrixParameters( void )
{
  itkExceptionMacro(
   <<
    "Setting the matrix of a ScaleVersor3D transform is not supported at this time." );
}
  ...
  GetJacobian( const InputPointType & p ) const
{
  ...
  this->m_Jacobian[0][0] = 2.0 * ( ( vyw + vxz ) * py + ( vzw - vxy ) * pz )
                           / vw;
  this->m_Jacobian[1][0] = 2.0
                           * ( ( vyw
                                 - vxz ) * px   - 2 * vxw   * py
                              + ( vxx - vww ) * pz )
                           / vw;
  this->m_Jacobian[2][0] = 2.0
                           * ( ( vzw
                                 + vxy ) * px
                              + ( vww - vxx ) * py   - 2 * vxw   * pz )
                           / vw;

  this->m_Jacobian[0][1] = 2.0
                           * ( -2 * vyw  * px
                               + ( vxw + vyz ) * py + ( vww - vyy ) * pz )
                           / vw;
  this->m_Jacobian[1][1] = 2.0
                           * ( ( vxw
                                 - vyz ) * px
                              + ( vzw + vxy ) * pz )
                           / vw;
  this->m_Jacobian[2][1] = 2.0
                           * ( ( vyy
                                 - vww ) * px
                              + ( vzw - vxy ) * py   - 2 * vyw   * pz )
                           / vw;

  this->m_Jacobian[0][2] = 2.0
                           * ( -2 * vzw  * px
                               + ( vzz - vww ) * py + ( vxw - vyz ) * pz )
                           / vw;
  this->m_Jacobian[1][2] = 2.0
                           * ( ( vww
                                 - vzz ) * px   - 2 * vzw   * py
                              + ( vyw + vxz ) * pz )
                           / vw;
