...
set ::ITK_CABLE_DATE "2009-08-17\\ 16:00"
set ::KWWidgets_DATE "2009-08-17"
set ::VTK_DATE "2009-06-18"
set ::vtkinria3d_TAG "20090805"
...
if { [string compare $::BUILD_PACKAGE(cmake) "yes" ] != 0 &&
     [string compare $patch_level "2.6-patch"] == 0 } {
...
set ::VTK_DIR  $::PACKAGE_LIB/VTK-build
...
set ::KWWIDGETS_DIR  $::PACKAGE_LIB/KWWidgets
set ::ITK_BINARY_PATH $::PACKAGE_LIB/Insight-build
set ::TCL_BIN_DIR $::PACKAGE_LIB/tcl-build/bin
set ::TCL_LIB_DIR $::PACKAGE_LIB/tcl-build/lib
set ::TCL_INCLUDE_DIR $::PACKAGE_LIB/tcl-build/include
set ::PYTHON_BIN_DIR $::PACKAGE_LIB/python-build
set ::SOV_BINARY_DIR ""
set ::XVNC_EXECUTABLE " "
...
#        set ::MAKE "make"
        ...
        switch $OSX_VERSION {
          ...
          "9" {
            ...
            ## set ::env(LDFLAGS) "-Wl,-dylib_file,/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib:/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib"
              set ::CMAKE_SHARED_LINKER_FLAGS "${LD_FLAGS} -Wl,-dylib_file,/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib:/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib"
            set ::CMAKE_MODULE_LINKER_FLAGS "${LD_FLAGS} -Wl,-dylib_file,/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib:/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib"
            set ::CMAKE_EXE_LINKER_FLAGS "${LD_FLAGS} -Wl,-dylib_file,/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib:/System/Library/Frameworks/OpenGL.framework/Versions/A/Libraries/libGL.dylib"
          }
        }
        ...
        "DEBUG_64" {
              set ::CC "$COMPILER_PATH/$C_COMPILER"
              set ::CXX "$COMPILER_PATH/$COMPILER"
              set ::CFLAGS   "-UNDEBUG -g -Wall -msse -mmmx -msse2 -msse3 -UITKIO_DEPRECATED_METADATA_ORIENTATION -m64 -arch x86_64 -Wconversion -mmacosx-version-min=10.4"
              set ::CXXFLAGS "-UNDEBUG -g -Wall -msse -mmmx -msse2 -msse3 -UITKIO_DEPRECATED_METADATA_ORIENTATION -m64 -arch x86_64 -Wconversion -mmacosx-version-min=10.4"
              set ::env(CC) $::CC
              set ::env(CXX) $::CXX
              ##  Some of the iwidgets, itcl, itk does not compile with these flags due to poor build tools used.
              set ::env(CFLAGS) $::CFLAGS
        ...
        "FAST_64" {
              set ::CC "$COMPILER_PATH/$C_COMPILER"
              set ::CXX "$COMPILER_PATH/$COMPILER"
              set ::CFLAGS   "-DNDEBUG -O2 -msse -mmmx -msse2 -msse3 -UITKIO_DEPRECATED_METADATA_ORIENTATION -m64 -arch x86_64 -Wconversion -mmacosx-version-min=10.4"
              set ::CXXFLAGS "-DNDEBUG -O2 -msse -mmmx -msse2 -msse3 -UITKIO_DEPRECATED_METADATA_ORIENTATION -m64 -arch x86_64 -Wconversion -mmacosx-version-min=10.4"
              set ::env(CC) $::CC
              set ::env(CXX) $::CXX
              ##  Some of the iwidgets, itcl, itk does not compile with these flags due to poor build tools used.
              set ::env(CFLAGS) $::CFLAGS
...
set ::COMPILER_HOSTNAME [regsub "\\.psychiatry\\.uiowa\\.edu" ${FULLHOSTNAME} ""]
    ...
    set ::CMAKE_COMPILE_SETTINGS [list "-G$GENERATOR" "-DCMAKE_CXX_COMPILER:FILEPATH=$::CXX" "-DCMAKE_CXX_FLAGS:STRING=$::CXXFLAGS" "-DCMAKE_CXX_FLAGS_RELEASE:STRING=$::CXXFLAGS" "-DCMAKE_CXX_FLAGS_DEBUG:STRING=$::CXXFLAGS" "-DCMAKE_C_COMPILER:FILEPATH=$::CC" "-DCMAKE_C_FLAGS:STRING=$::CFLAGS" "-DDART_TESTING_TIMEOUT:STRING=6500" "-DMEMORYCHECK_COMMAND_OPTIONS:STRING=$::VALGRIND_COMMANDS_FLAGS" "-DMEMORYCHECK_COMMAND:PATH=$::VALGRIND_EXEC" "-DCMAKE_SHARED_LINKER_FLAGS=$::CMAKE_SHARED_LINKER_FLAGS" "-DCMAKE_EXE_LINKER_FLAGS=$::CMAKE_EXE_LINKER_FLAGS" "-DCMAKE_MODULE_LINKER_FLAGS=$::CMAKE_MODULE_LINKER_FLAGS" "-DSITE:STRING=$::COMPILER_HOSTNAME.uiowa" "-DBUILDNAME:STRING=$::tcl_platform(os).$env(PACKAGE_BUILD_TYPE)"]
