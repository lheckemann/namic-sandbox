...
#include <fstream>
#include "BRAINSFit2DHelperTemplate.h"
...
namespace itk
{
        ...
        if ( transformType[l] == "Rigid" )
          ...
          if ( CurrentTransformRank <= 1 )
            ...
            std::cerr << "Ordering of transforms does not proceed from\n"
              ...
              << "Rigid < ScaleVersor2D < ScaleSkewVersor3D < Affine < (BSpline | ROIBSpine)" << std::endl;
            exit (-1);
            }
          }
        else if ( transformType[l] == "ScaleVersor2D" )
          {
          if ( CurrentTransformRank <= 2 )
            ...
            std::cerr << "Ordering of transforms does not proceed from\n"
              << "smallest to largest.  Please review settings for transformType.\n"
              << "Rigid < ScaleVersor2D < ScaleSkewVersor2D < Affine < (BSpline | ROIBSpline)" << std::endl;
            exit (-1);
            }
          }
        else if ( transformType[l] == "ScaleSkewVersor2D" )
          {
          ...
          if ( CurrentTransformRank <= 4 )
            ...
            std::cerr << "Ordering of transforms does not proceed from\n"
              << "smallest to largest.  Please review settings for transformType.\n"
              << "Rigid < ScaleVersor2D < ScaleSkewVersor3D < Affine < (BSpline | ROIBSpline)" << std::endl;
            exit (-1);
            }
          }
          ...
          std::cerr << "Ordering of transforms must proceed from\n"
            << "smallest to largest.  Please review settings for transformType.\n"
            << "Rigid < ScaleVersor2D < ScaleSkewVersor3D < Affine < BSpline" << std::endl;
          exit(-1);
          }
        }
      }
      ...
      typename TransformType::Pointer DoCenteredInitialization(
        ...
        if ( initializeTransformMode == "useGeometryAlign" )
          ...
          if ( initializeTransformMode == "useCenterOfROIAlign" )
            ...
            typedef itk::StatisticsLabelObject< unsigned char, 2 > LabelObjectType;
          ...
          itk::Vector< double, 2 > scaleValue;
  ...
  template <class FixedImageType, class MovingImageType>
    BRAINSFit2DHelperTemplate<FixedImageType,MovingImageType>::BRAINSFit2DHelperTemplate()
      {
  ...
  template <class FixedImageType, class MovingImageType>
    ...
    void
    BRAINSFit2DHelperTemplate<FixedImageType, MovingImageType>::
  ...
  template <class FixedImageType, class MovingImageType>
    void
    BRAINSFit2DHelperTemplate<FixedImageType, MovingImageType>::StartRegistration(void)
      {
                ...
                if ( checkMatch )
                  ...
                  if ( initialBSplineTransform->GetNumberOfParameters() ==
                    ...
                    std::cerr
                      ...
                      << std::endl;
                    std::cerr
                      << "BRAINSFit2D was only able to use the bulk transform that was before it."
                      << std::endl;
                    exit(-1);
                    }
                  }
  ...
  template <class FixedImageType, class MovingImageType>
    void
    BRAINSFit2DHelperTemplate<FixedImageType, MovingImageType>::PrintSelf(std::ostream & os, Indent indent) const
      {
  ...
  template <class FixedImageType, class MovingImageType>
    void
    BRAINSFit2DHelperTemplate<FixedImageType, MovingImageType>::GenerateData()
      {
