...
::Execute()
{
  ...
  if ( m_InitialDeformationFieldFilename != "" )
    ...
    // in mm
    typedef itk::AffineTransform< double, 2 > ITKAffineTransformType;
    typedef itk::Vector< double, 2 >          VectorType;
    VectorType const fixedImageScaleReciprocal( Reciprocal< double, 2 >(
                                                  transform.GetFixedImageSpacing() ) );

    VectorType const movingImageScale( transform.GetMovingImageSpacing() );

    typedef CrossOverAffineSystem< double, 2 > CrossOverAffineSystemType;
