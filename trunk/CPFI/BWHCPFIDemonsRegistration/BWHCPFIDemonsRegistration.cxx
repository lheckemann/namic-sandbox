/**
 * This code is based on Tom Vercauteren's Diffeomorphic demons alogrithm
 * commandlie interface. The code is modified to support 2D image for
 * BWH CPFI team.
 *
 */
...
#include "BWHCPFIDemonsRegistrationCLP.h"
...
void DoDemonsRegistration( arguments args )
{
   ...
   const unsigned int ImageDimension = Dimension;

   // read and transform oriented images, operate on non-oriented
   typedef itk::OrientedImage< PixelType, FileImageDimension >  FileOrientedImageType;
   typedef itk::OrientedImage< PixelType, ImageDimension >  OrientedImageType;
   typedef itk::OrientImageFilter<FileOrientedImageType,FileOrientedImageType> OrientFilterType;//##
   typedef itk::ExtractImageFilter<FileOrientedImageType, OrientedImageType>   InputExtractFilterType;
   typedef itk::TileImageFilter<OrientedImageType, FileOrientedImageType> OutputPasteFilterType;
   typedef itk::Image< PixelType, ImageDimension > ImageType;

   typedef itk::ImageFileWriter< FileOrientedImageType >  WriterType;
   ...
   typedef typename itk::ImageFileReader< FileOrientedImageType > ImageReaderType;
  ...
  // into the fixed image space (probably more efficient)
   ...
   // ------------------------------------------------------------
   // Extract Filter for collapsing image dimension (3D->2Din case of 2D registration)
   ...
   {   
      ...
      matcher->SetInput( movingExtract->GetOutput() );
      matcher->SetReferenceImage( fixedExtract->GetOutput() );
   ...
   typedef itk::CastImageFilter
     < ImageType, OrientedImageType > CastFilterType;
   ...
   outputPaste->SetInput(caster->GetOutput());
   outputPaste->Update();
   ...
   if (ImageDimension == 2)
     {
       // Flip axis
       typedef typename itk::FlipImageFilter< FileOrientedImageType > FlipFilterType;
       ...
       typename FileOrientedImageType::Pointer outputImage = FileOrientedImageType::New();
       outputImage = outputFlip->GetOutput();
       outputImage->Update();
       
       outputImage->SetOrigin(fixedImageReader->GetOutput()->GetOrigin());
       typename FileOrientedImageType::DirectionType direction;
