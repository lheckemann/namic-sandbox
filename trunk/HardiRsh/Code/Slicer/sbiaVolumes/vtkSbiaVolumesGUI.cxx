...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkSbiaVolumesGUI );
vtkCxxRevisionMacro (vtkSbiaVolumesGUI, "$Revision: 1.0 $");

//---------------------------------------------------------------------------
vtkSbiaVolumesGUI::vtkSbiaVolumesGUI ( )
{
  this->Logic = NULL;
...
vtkSbiaVolumesGUI::~vtkSbiaVolumesGUI ( )
{
  ...
  if (this->RSHVDW) 
    {
    this->RSHVDW->RemoveMRMLObservers();
    this->RSHVDW->SetParent(NULL );
    this->RSHVDW->Delete ( );
    }
  ...
  if (this->RSHDisplayFrame)
    {
    this->RSHDisplayFrame->SetParent(NULL );
    this->RSHDisplayFrame->Delete( );
    }
  if ( this->HelpFrame )
    {
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::PrintSelf ( ostream& os, vtkIndent indent )
{
  this->vtkObject::PrintSelf ( os, indent );

  os << indent << "SbiaVolumesGUI: " << this->GetClassName ( ) << "\n";
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::RemoveGUIObservers ( )
{
  if (this->VolumeSelectorWidget)
    {
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::AddGUIObservers ( )
{
  // Fill in
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::ProcessGUIEvents(vtkObject *caller, unsigned long event, void *callData )
{
  if (this->VolumeFileHeaderWidget == vtkSlicerVolumeFileHeaderWidget::SafeDownCast(caller) && 
    ...
    vtkSbiaVolumesLogic* volumeLogic = this->Logic;
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::ProcessLogicEvents ( vtkObject *caller, unsigned long event, void *callData )
{
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::ProcessMRMLEvents(vtkObject *caller, unsigned long event, void *callData )
{
}

//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateModuleEventBindings ( )
{
}

//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::ReleaseModuleEventBindings ( )
{
}

//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::Enter ( vtkMRMLNode *node )
{
  if ( this->Built == false )
    {
    this->BuildGUI();
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::Exit ( )
{
  this->ReleaseModuleEventBindings();
}

//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::UpdateFramesFromMRML()
{

  vtkMRMLVolumeNode *refNode = 
    vtkMRMLVolumeNode::SafeDownCast(this->VolumeSelectorWidget->GetSelected());

  std::cerr << "vtkSbiaVolumesGUI::UpdateFramesFromMRML:\n";
  ...
  if (refNode != NULL)
    ...
    if ( !strcmp(refNode->GetClassName(), "vtkMRMLScalarVolumeNode"))
      ...
      //DTI VOLUME NO MEASUREMENT FRAME  
      this->GradientFrame->EnabledOff();
      this->GradientFrame->SetAllowFrameToCollapse(0);
    ...
    else if ( !strcmp(refNode->GetClassName(), "vtkMRMLDiffusionRSHVolumeNode") )
      { 
      if (this->VolumeDisplayWidget == NULL || this->VolumeDisplayWidget != RSHVDW)
        {
        if (this->VolumeDisplayWidget != NULL)
          {
          this->VolumeDisplayWidget->TearDownWidget();
        ...
        tearDown = 1;
        this->CreateRSHDisplayWidget();
        this->VolumeDisplayWidget = this->RSHVDW;
        this->VolumeDisplayFrame = this->RSHDisplayFrame;
        }
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::TearDownGUI ( )
{
  this->Exit();
  if ( this->Built )
    {
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::BuildGUI ( )
{
  ...
  this->RSHDisplayFrame = vtkKWFrame::New();
  this->RSHDisplayFrame->SetParent( this->DisplayFrame->GetFrame() );
  this->RSHDisplayFrame->Create( );
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateScalarDisplayWidget ( )
{
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateVectorDisplayWidget ( )
{
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateLabelMapDisplayWidget ( )
{
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateDWIDisplayWidget ( )
{
  if (this->dwiVDW == NULL)
    {
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateDTIDisplayWidget ( )
{
  if (this->dtiVDW == NULL)
    {
...
//---------------------------------------------------------------------------
void vtkSbiaVolumesGUI::CreateRSHDisplayWidget ( )
{
  if (this->RSHVDW == NULL)
    {
    this->RSHVDW = vtkSlicerDiffusionRSHVolumeDisplayWidget::New( );
    this->RSHVDW->SetParent( this->RSHDisplayFrame );
    this->RSHVDW->SetMRMLScene(this->GetMRMLScene());
    this->RSHVDW->AddMRMLObservers();
    this->RSHVDW->Create();
    this->Script ( "pack %s -side top -anchor nw -fill x -padx 2 -pady 2 -in %s",
                   RSHVDW->GetWidgetName(), this->RSHDisplayFrame->GetWidgetName());    
    }
}

void vtkSbiaVolumesGUI::CopyTagAndValues( vtkITKArchetypeImageSeriesReader* reader )
{
...
unsigned int vtkSbiaVolumesGUI::GetNumberOfItemsInDictionary()
{
  return this->Tags.size();
}

bool vtkSbiaVolumesGUI::HasKey( char* tag )
{
...
const char* vtkSbiaVolumesGUI::GetNthKey( unsigned int n )
{
  if (n >= this->Tags.size())
    {
    return NULL;
...
const char* vtkSbiaVolumesGUI::GetNthValue( unsigned int n )
{
...
const char* vtkSbiaVolumesGUI::GetTagValue( char* tag )
{
  std::string tagstr (tag);
