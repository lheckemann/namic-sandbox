/*=auto=========================================================================
  ...
  Program:   3D Slicer
  Module:    $RCSfile: vtkSbiaVolumesLogic.cxx,v $
...
//#include "vtkDiffusionRSHMathematics.h"
...
vtkCxxRevisionMacro(vtkSbiaVolumesLogic, "$Revision: 1.9.12.1 $");
vtkStandardNewMacro(vtkSbiaVolumesLogic);

//----------------------------------------------------------------------------
vtkSbiaVolumesLogic::vtkSbiaVolumesLogic()
{
...
//----------------------------------------------------------------------------
void vtkSbiaVolumesLogic::ProcessMRMLEvents(vtkObject *caller, 
                                            unsigned long event, 
                                            void *callData)
{
}
...
//----------------------------------------------------------------------------
void vtkSbiaVolumesLogic::ProcessLogicEvents(vtkObject *caller, 
                                            unsigned long event, 
                                            void *callData)
{
...
//#include "vtkMRMLScalarVolumeDisplayNode.h"
//----------------------------------------------------------------------------
void vtkSbiaVolumesLogic::SetActiveVolumeNode(vtkMRMLVolumeNode *activeNode)
{
...
vtkMRMLVolumeNode* vtkSbiaVolumesLogic::AddHeaderVolume (const char* filename, const char* volname, 
...
//----------------------------------------------------------------------------
vtkMRMLScalarVolumeNode* vtkSbiaVolumesLogic::AddArchetypeScalarVolume (const char* filename, const char* volname, int loadingOptions)
{ 
  int centerImage = 0;
  int labelMap = 0;
  int singleFile = 0;
...
// higher bits are reserved for future use
vtkMRMLVolumeNode* vtkSbiaVolumesLogic::AddArchetypeVolume (const char* filename, const char* volname, int loadingOptions, vtkStringArray *fileList)
{
  int centerImage = 0;
  int labelMap = 0;
  int singleFile = 0;
  ...
  vtkMRMLDiffusionRSHVolumeDisplayNode *qbdisplayNode = NULL;
  ...
  vtkMRMLDiffusionRSHVolumeNode *RSHNode = vtkMRMLDiffusionRSHVolumeNode::New();
  ...
  RSHNode->SetName(volumeName.c_str());
  ...
  // set up the RSH node's support nodes
  qbdisplayNode = vtkMRMLDiffusionRSHVolumeDisplayNode::New();
  qbdisplayNode->SetWindow(0);
  qbdisplayNode->SetLevel(0);
  qbdisplayNode->SetUpperThreshold(0);
  qbdisplayNode->SetLowerThreshold(0);
  qbdisplayNode->SetAutoWindowLevel(1);
  this->GetMRMLScene()->AddNodeNoNotify(qbdisplayNode);
  this->GetMRMLScene()->AddNodeNoNotify(RSHNode);
  RSHNode->SetAndObserveStorageNodeID(storageNode1->GetID());
  RSHNode->SetAndObserveDisplayNodeID(qbdisplayNode->GetID());
  ...
  else if (storageNode1->ReadData(RSHNode))
    {
    vtkDebugMacro("RSH HAS BEEN READ");    
    displayNode = qbdisplayNode;
    qbdisplayNode->AddSliceGlyphDisplayNodes(RSHNode);
    volumeNode = RSHNode;
    storageNode = storageNode1;
    nodeSetUsed = 6;
    }
  ...
  if (nodeSetUsed != 6)
    {
    RSHNode->SetAndObserveDisplayNodeID(NULL);
    RSHNode->SetAndObserveStorageNodeID(NULL);
    this->GetMRMLScene()->RemoveNodeNoNotify(qbdisplayNode);
    this->GetMRMLScene()->RemoveNodeNoNotify(RSHNode);
    RSHNode->Delete(); RSHNode = NULL;
    qbdisplayNode->Delete(); qbdisplayNode = NULL;
    }
...
//----------------------------------------------------------------------------
int vtkSbiaVolumesLogic::SaveArchetypeVolume (const char* filename, vtkMRMLVolumeNode *volumeNode)
{
  ...
  if (volumeNode->IsA("vtkMRMLDiffusionWeightedVolumeNode") ||
      ...
      volumeNode->IsA("vtkMRMLDiffusionRSHVolumeNode") ||
...
//----------------------------------------------------------------------------
vtkMRMLScalarVolumeNode *vtkSbiaVolumesLogic::CreateLabelVolume (vtkMRMLScene *scene, vtkMRMLVolumeNode *volumeNode, const char *name)
{
  if ( volumeNode == NULL ) 
    {
    return NULL;
...
//----------------------------------------------------------------------------
void vtkSbiaVolumesLogic::PrintSelf(ostream& os, vtkIndent indent)
{
  this->vtkObject::PrintSelf(os, indent);

  os << indent << "vtkSbiaVolumesLogic:             " << this->GetClassName() << "\n";
...
//----------------------------------------------------------------------------
int vtkSbiaVolumesLogic::IsFreeSurferVolume (const char* filename)
{
...
//-------------------------------------------------------------------------
void vtkSbiaVolumesLogic::ComputeTkRegVox2RASMatrix ( vtkMRMLVolumeNode *VNode,
                                                                       vtkMatrix4x4 *M )
{
    double dC, dS, dR;
    double Nc, Ns, Nr;
...
//-------------------------------------------------------------------------
void vtkSbiaVolumesLogic::TranslateFreeSurferRegistrationMatrixIntoSlicerRASToRASMatrix( vtkMRMLVolumeNode *V1Node,
