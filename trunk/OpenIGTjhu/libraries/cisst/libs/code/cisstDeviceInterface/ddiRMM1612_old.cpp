...
CMN_IMPLEMENT_SERVICES(ddiRMM1612);


ddiRMM1612::ddiRMM1612(const char* deviceName):
    ddiDeviceInterface(deviceName)
{
  ...
  */
  ...
  DDI_ADD_METHOD_COMMAND1(ddiRMM1612, SetOutputAll);
...
void ddiRMM1612::Configure(const char *filename) {
    
    cmnXMLPath xmlConfig;
    if (!filename) {
  CMN_LOG_CLASS(2) << "Warning, could not configure RMM1612 device" << std::endl;
  return;
    }
    ...
    CMN_LOG_CLASS(3) << "Configuring RMM1612 Board with \"" << filename << "\"" << std::endl;
  ...
  //    BaseAddress = 0x200 + SelectorSwitchPosition * 0x20;
    ...
    //DACRangeLow = 0;
    //DACRangeHigh = 10;
    xmlConfig.GetXMLValue(context.c_str(), "@DACLowRange", DACRangeLow);
    xmlConfig.GetXMLValue(context.c_str(), "@DACHighRange", DACRangeHigh);
    CMN_LOG_CLASS(3) << "BaseAddress: " << BaseAddress << std::endl;
    CMN_LOG_CLASS(3) << "DAC Min. Voltage: " << DACRangeLow << " ,  DAC Max. Voltage: " << DACRangeHigh << std::endl;
...
sprintf(path, "Axis[%d]/ADC/@CountsToMotorCurrents", axis+1);
  ...
  */
  ...
  dscdacs.output_codes = (DSCDACODE*)malloc( sizeof(DSCDACODE) * 16 );
    ...
    CMN_LOG_CLASS(3) << "Configured RMM1612" << std::endl;
}
...
//GSF
DDI_BEGIN_IMPLEMENT_COMMAND1_IN(ddiRMM1612, SetOutputAll)
    ...
    for(int i=0; i<16; i++){
      ...
      //  pdata->Data[i] = 0;
        ...
        DAC_voltage[i] = pdata->Data[i];
        DAC_raw[i] = floor(( DAC_voltage[i] / (DACRangeHigh - DACRangeLow) ) * 4095 + 0.5);
            
        // Set output values, Not transfered to DAC yet
