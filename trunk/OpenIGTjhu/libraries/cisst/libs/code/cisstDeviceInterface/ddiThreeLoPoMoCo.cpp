...
CMN_IMPLEMENT_SERVICES(ddiThreeLoPoMoCo);


ddiThreeLoPoMoCo::ddiThreeLoPoMoCo(const char* deviceName,
...
ddiThreeLoPoMoCo::~ddiThreeLoPoMoCo()
{
}


void ddiThreeLoPoMoCo::Configure(const char* filename)
{
    CMN_LOG_CLASS(3) << "Configuring ddiThreeLoPoMoCo" << std::endl;

    // if no file name has been provided, use defaults
    if (!filename) {
  CMN_LOG_CLASS(2) << "Configuring ddiThreeLoPoMoCo with default LoPoMoCo{1,2,3}.xml"
       <<std::endl;
  ...
  return;
    ...
    // load configuration file for ddiThreeLoPoMoCo
    CMN_LOG_CLASS(3) << "Configuring ddiThreeLoPoMoCo with file: " << filename << std::endl;
  ...
  CMN_LOG_CLASS(1) << "Couldn't find all the required ConfigFile in " << filename << std::endl;
    ...
    CMN_LOG_CLASS(3) << "Configuring ddiThreeLoPoMoCo finished" << std::endl;
}
