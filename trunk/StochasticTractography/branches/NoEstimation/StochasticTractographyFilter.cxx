...
int main(int argc, char* argv[]){
  ...
  typedef itk::Image< DiffusionTensor3D< double > > DTIImageType;
  typedef itk::Image< unsigned int, 3 > b0ImageType;
  ...
  typedef itk::ImageFileReader< DTIImageType > DTIImageReaderType;
  typedef itk::ImageFileReader< b0ImageType > b0ImageReaderType;
  ...
  //define a stochastic tractography filter type and associated bValue,
  //gradient direction, and measurement frame types
  typedef itk::StochasticTractographyFilter< DTIImageType, b0ImageType, DWIVectorImageType, WMPImageType,
    CImageType > STFilterType;
 ...
 // }
  ...
  //read in the DTI image
  DTIImageReaderType::Pointer dtireaderPtr = DTIImageReaderType::New();
  dtireaderPtr->SetFileName( dtifilename );
  dtireaderPtr->Update();
  
  //read in the b0 image
  b0ImageReaderType::Pointer b0readerPtr = b0ImageReaderType::New();
  b0readerPtr->SetFileName( b0filename );
  b0readerPtr->Update();
  ...
  */
  ...
  stfilterPtr->SetInput( dtireaderPtr->GetOutput() );
  stfilterPtr->SetDWIImageInput( dwireaderPtr->GetOutput() );
  stfilterPtr->Setb0ImageInput( b0readerPtr->GetOutput() );
