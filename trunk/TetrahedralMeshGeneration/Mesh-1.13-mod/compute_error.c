                           ...
                           int **dcl_buf, int *dcl_buf_sz)
{
  ...
  p_rel.x = p.x-bbox_min.x;
  p_rel.y = p.x-bbox_min.y;
  p_rel.z = p.x-bbox_min.z;
  /*
  fprintf(stderr, "Point coordinates: %g %g %g \n",
    p.x, p.y, p.z);
  fprintf(stderr, "Bbox coordinates: %g %g %g \n",
    bbox_min.x, bbox_min.y, bbox_min.z);
  fprintf(stderr, "Relative coordinates: %g %g %g \n",
    p_rel.x, p_rel.y, p_rel.z);
  fprintf(stderr, "Relative coordinates: %g %g %g \n",
    p.x-bbox_min.x, p.y-bbox_min.y, p.z-bbox_min.z);
  */
  ...
  grid_coord.x = (int) floor((p.x-bbox_min.x)/cell_sz);
  if (grid_coord.x < 0) {
    grid_coord.x = 0;
  } else if (grid_coord.x >= grid_sz.x) {
    grid_coord.x = grid_sz.x-1;
  }
  grid_coord.y = (int) floor((p.y-bbox_min.y)/cell_sz);
  if (grid_coord.y < 0) {
    grid_coord.y = 0;
  } else if (grid_coord.y >= grid_sz.y) {
    grid_coord.y = grid_sz.y-1;
  }
  grid_coord.z = (int) floor((p.z-bbox_min.z)/cell_sz);
  if (grid_coord.z < 0) {
    grid_coord.z = 0;
  ...
  /*
  fprintf(stderr, "Grid coordinates: %i %i %i \n",
