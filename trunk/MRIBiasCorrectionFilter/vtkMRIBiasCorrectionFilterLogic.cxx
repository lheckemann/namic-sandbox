...
Program:   3D Slicer
Module:    $RCSfile: vtkMRIBiasCorrectionFilterLogic.cxx,v $
...
//#include "vtkMRIBiasCorrectionFilterGUI.h"
...
vtkMRIBiasCorrectionFilterLogic* vtkMRIBiasCorrectionFilterLogic::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkMRIBiasCorrectionFilterLogic");
  if(ret)
    {
      return (vtkMRIBiasCorrectionFilterLogic*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkMRIBiasCorrectionFilterLogic;
}


//----------------------------------------------------------------------------
vtkMRIBiasCorrectionFilterLogic::vtkMRIBiasCorrectionFilterLogic()
{
  this->MRIBiasCorrectionFilterNode = NULL;
}

//----------------------------------------------------------------------------
vtkMRIBiasCorrectionFilterLogic::~vtkMRIBiasCorrectionFilterLogic()
{
  vtkSetMRMLNodeMacro(this->MRIBiasCorrectionFilterNode, NULL);
}

//----------------------------------------------------------------------------
void vtkMRIBiasCorrectionFilterLogic::PrintSelf(ostream& os, vtkIndent indent)
{
  
}

void vtkMRIBiasCorrectionFilterLogic::Apply()
{

  // check if MRML node is present 
  if (this->MRIBiasCorrectionFilterNode == NULL)
    {
  ...
  // find input volume
    vtkMRMLScalarVolumeNode *inVolume = vtkMRMLScalarVolumeNode::SafeDownCast(this->GetMRMLScene()->GetNodeByID(this->MRIBiasCorrectionFilterNode->GetInputVolumeRef()));
  if (inVolume == NULL)
    {
    ...
    return;
     ...
     // find mask volume
  vtkMRMLScalarVolumeNode *maskVolume =  vtkMRMLScalarVolumeNode::SafeDownCast(this->GetMRMLScene()->GetNodeByID(this->MRIBiasCorrectionFilterNode->GetMaskVolumeRef()));
  if (maskVolume == NULL)
    {
    vtkErrorMacro("No mask volume found with id= " << this->MRIBiasCorrectionFilterNode->GetMaskVolumeRef());
    return;
    }
  
  // find output volume
  vtkMRMLScalarVolumeNode *outVolume =  vtkMRMLScalarVolumeNode::SafeDownCast(this->GetMRMLScene()->GetNodeByID(this->MRIBiasCorrectionFilterNode->GetOutputVolumeRef()));
  if (outVolume == NULL)
    {
    vtkErrorMacro("No output volume found with id= " << this->MRIBiasCorrectionFilterNode->GetOutputVolumeRef());
    return;
    }
...
// processing                               
  ...
  std::cout<<"shrink factor: "<<this->MRIBiasCorrectionFilterNode->GetShrink()<< std::endl;
  
  shrinker->SetShrinkFactors( this->MRIBiasCorrectionFilterNode->GetShrink());
  maskshrinker->SetShrinkFactors(this->MRIBiasCorrectionFilterNode->GetShrink());
  ...
  std::cout<<"number of iteration: "<<this->MRIBiasCorrectionFilterNode->GetMax()<< std::endl;
    correcter->SetMaximumNumberOfIterations( this->MRIBiasCorrectionFilterNode->GetMax() );

  std::cout<<"number of fitting level: "<<this->MRIBiasCorrectionFilterNode->GetNum()<< std::endl;
    correcter->SetNumberOfFittingLevels(this->MRIBiasCorrectionFilterNode->GetNum());
    
  std::cout<<"Wiener Filter Noise: "<<this->MRIBiasCorrectionFilterNode->GetWien()<< std::endl;
    correcter->SetWeinerFilterNoise(this->MRIBiasCorrectionFilterNode->GetWien());
    
  std::cout<<"Bias Field Full..: "<<this->MRIBiasCorrectionFilterNode->GetField()<< std::endl;
    correcter->SetBiasFieldFullWidthAtHalfMaximum(this->MRIBiasCorrectionFilterNode->GetField());
    
   std::cout<<"Convergence Threshold: "<<this->MRIBiasCorrectionFilterNode->GetCon()<< std::endl;   
    correcter->SetConvergenceThreshold(this->MRIBiasCorrectionFilterNode->GetCon());
...
void vtkMRIBiasCorrectionFilterLogic::SliceProcess(vtkTransform* xyToijk,double dim0,double dim1)
{
 // check if MRML node is present 
  if (this->MRIBiasCorrectionFilterNode == NULL)
    {
    vtkErrorMacro("No input MRIBiasCorrectionFilterNode found");
    return;
    }
  ...
  vtkMRMLScalarVolumeNode *stoVolume =  vtkMRMLScalarVolumeNode::SafeDownCast(this->GetMRMLScene()->GetNodeByID(this->MRIBiasCorrectionFilterNode->GetStorageVolumeRef()));
  if (stoVolume == NULL)
    {
    vtkErrorMacro("No storage volume found with id= " << this->MRIBiasCorrectionFilterNode->GetStorageVolumeRef());
    return;
    }
...
void vtkMRIBiasCorrectionFilterLogic::ApplyPreview(double red, double yellow, double green,vtkImageData* image)
{
 // check if MRML node is present 
  if (this->MRIBiasCorrectionFilterNode == NULL)
    {
...
/*
  ...
  applicationLogic->GetSelectionNode()->SetReferenceActiveVolumeID(this->MRIBiasCorrectionFilterNode->GetOutputVolumeRef());
...
VTK2ITKConnectorFilterType::New();
 ...
 std::cout<<"UPDATE: "<<this->MRIBiasCorrectionFilterNode->GetCon()<< std::endl;   
    correcter->SetConvergenceThreshold(this->MRIBiasCorrectionFilterNode->GetCon());
...
void vtkMRIBiasCorrectionFilterLogic::Preview()
{
 // check if MRML node is present 
  if (this->MRIBiasCorrectionFilterNode == NULL)
    {
 ...
 this->ImageThreshold->ThresholdByUpper(this->MRIBiasCorrectionFilterNode->GetConductance()); 
...
void vtkMRIBiasCorrectionFilterLogic::Cut()
{
  // check if MRML node is present 
  if (this->MRIBiasCorrectionFilterNode == NULL)
    {
...
this->CurrentSlide->SetOrigin(0,0,0);  
  */
  ...
  //vtkMRMLMRIBiasCorrectionFilterNode* n = this->GetMRIBiasCorrectionFilterNode();
  /*
  std::cout << "Storage node ID: " << this->GetMRMLScene()->GetNodeByID(this->MRIBiasCorrectionFilterNode->GetStorageVolumeRef()) << std::endl;
...
int vtkMRIBiasCorrectionFilterLogic::InitMaxThreshold()
{
...
int vtkMRIBiasCorrectionFilterLogic::InitMinThreshold()
{
...
int vtkMRIBiasCorrectionFilterLogic::AxialMin()
{
...
int vtkMRIBiasCorrectionFilterLogic::AxialMax()
{
...
int vtkMRIBiasCorrectionFilterLogic::SagittalMax()
{
...
int vtkMRIBiasCorrectionFilterLogic::CoronalMax()
{
