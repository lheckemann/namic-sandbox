...
//      (m_TLEN * Z*Z) has a Chi-squared-m_TLEN density. Now, a var with
//      an approximate Chi-sq-K distn can be got as
//          0.5 * (C + A*n)**2  where n has unit Normal distn,
//      A = (1 + 1 / (8K)),  C*C = 2K - A*A    (For large K)
//          So we form Z as (sqrt (1 / 2m_TLEN)) * (C + A*n)
//      or:
//          Z = (sqrt (1/2m_TLEN)) * A * (B + n)
//      where:
//          B = C / A.
//      We set m_Chic1 = A * vcl_sqrt(0.5 / m_TLEN),  m_Chic2 = B
...
double NormalVariateGenerator::FastNorm(void)
{
 ...
 startpass:
  ...
  /*    We need m_ELEN to fix initial index into m_LEN, m_ELEN-1 to fix an odd
        stride, 2 to fix matrix type and maybe 1 for scantype, making
        2*m_ELEN + 2 in all, and leaving 29 - 2*m_ELEN unused
  */
 ...
 endpass:
  /*    Choose a value for m_GScale which will make the sum-of-squares have
        the variance of Chi-Sq (m_TLEN), i.e., 2*m_TLEN.  Choose a value from
        Chi-Sq (m_TLEN) using the method descibed in initnorm.
        The Normal variate is obtained from m_GaussSave[m_TLEN-1], which is
 ...
 nextpair:
  ...
  /*    Calc correction factor to make sum of squares = m_TLEN    */
 ...
 recalcsumsq:
  ...
  /*    Now ts should be m_Scale*m_Scale*m_TLEN or thereabouts   */
