...
TVoxelVolume::TVoxelVolume(const int dim[3], const unsigned char* data)
{
   ...
   data_ = new unsigned char[dim_[0]*dim_[1]*dim_[2]];
   assertStr(data_ != 0, "TVoxelVolume::TVoxelVolume : new returned 0");
   if (data == 0)
     this->Fill(0);
   else
     memcpy(data_, data, sizeof(unsigned char)*dim_[0]*dim_[1]*dim_[2]);
...
void TVoxelVolume::CopyToAVS(unsigned char* outavs)
{
  memcpy(outavs, data_, sizeof(unsigned char) * size_ );
}

void TVoxelVolume::CopyFromAVS(const unsigned char* inavs)
{
  memcpy(data_, inavs, sizeof(unsigned char) * size_ );
}
