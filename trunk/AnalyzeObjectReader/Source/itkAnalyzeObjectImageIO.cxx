...
{ 
  ...
  if ((magic_number1 != 'B')||(magic_number2 != 'M'))
    {
    std::cout << "AnalyzeObjectImageIO : Magic Number Fails = " << magic_number1 << " : " << magic_number2 << std::endl;
    inputStream.close();
    return false;
    }
...
void AnalyzeObjectImageIO::ReadImageInformation()
{
  ...
  if( m_Ifstream.fail() )
    {
    itkExceptionMacro("AnalyzeObjectImageIO could not open file: "
...
::Write( const void* buffer) 
{
  ...
  if(nDims != 2)
    {
    itkExceptionMacro(<< "AnalyzeObjectImageIO cannot write images with a dimension != 2");
    }
  ...
  if( (this->m_NumberOfComponents != 1) && (this->m_NumberOfComponents !=3) )
    {
    itkExceptionMacro(<<"AnalyzeObjectImageIO supports 1 or 3 components only");
    }
