...
vtkNeuroNavLogic::vtkNeuroNavLogic()
{
  ...
  this->ThreaderID = -1;
  this->ThreaderID = this->m_Threader->SpawnThread(vtkNeuroNavLogic::BeepingFunction,this);

  this->beepingMode = false;
  this->DestructorCalled = false;

  this->BeepingModeLock = itk::MutexLock::New();
}
...
vtkNeuroNavLogic::~vtkNeuroNavLogic()
{
  ...
  if(this->ThreaderID != -1)
    {
    this->m_Threader->TerminateThread(this->ThreaderID);
    this->ThreaderID = -1;
    }
...
int vtkNeuroNavLogic::EnableLocatorDriver(int sw)
{
  ...
  if (sw == 1)  // turn on
    {
    locatorDisp->SetVisibility(1);
    }
  else  // turn off
    {
    locatorDisp->SetVisibility(0);
    }
...
//---------------------------------------------------------------------------
ITK_THREAD_RETURN_TYPE vtkNeuroNavLogic::BeepingFunction(void* pInfoStruct)
{
  vtkNeuroNavLogic *NeuroNavLogic = (vtkNeuroNavLogic*) (((itk::MultiThreader::ThreadInfoStruct *)(pInfoStruct))->UserData);

  while(!NeuroNavLogic->DestructorCalled)
    {
    if(NeuroNavLogic->GetBeepingMode())
      {
#ifdef _WIN32
      Beep(500,250);
      Sleep(500);
...
void vtkNeuroNavLogic::SetBeepingMode(bool beep)
{
  this->BeepingModeLock->Lock();
  this->beepingMode = beep;
  this->BeepingModeLock->Unlock();
}

bool vtkNeuroNavLogic::GetBeepingMode()
{
  return this->beepingMode;
}
