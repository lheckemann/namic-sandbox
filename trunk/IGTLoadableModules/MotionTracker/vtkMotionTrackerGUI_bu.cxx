...
if (this->switchImage) {
 ...
 // 6/12/2011 ayamada
 // for get image dat from OpenCV IplIlmage 
 double *origin;
 double *spacing;
 int ncomp;
 int svoffset[] = {0,0,0};
int scalarType;
 ...
 int vImageSize[3];
 //imageData->GetExtent(vImageSize[0],vImageSize[1],vImageSize[2],vImageSize[3],vImageSize[4],vImageSize[5]);
...
spacing = imageData->GetSpacing();
 ...
 dsize = this->imageSize.width*this->imageSize.height*3;
 //dsize = vImageSize[0]*vImageSize[1]*3;
 
 if((vImageSize[0] != this->imageSize.width || vImageSize[1] != this->imageSize.height) && this->makeImageFromScanner==0)
 {
 this->makeImageFromScanner = 1;

 this->imageSize.width = 256;//vImageSize[0];
 this->imageSize.height = 256;//vImageSize[1];
  
 this->ImageFromScannerTmp = cvCreateImage(this->imageSize, IPL_DEPTH_16U, 1);
 this->ImageFromScanner = cvCreateImage(this->imageSize, IPL_DEPTH_16U, 3);
 //this->RGBImage = cvCreateImage(this->imageSize, IPL_DEPTH_8U, 3);

 // 6/12/2011 ayamada 
 //cvSetSize 
 /*
 this->ImageFromScannerTmp->SetDimensions(newImageSize.width, newImageSize.height, 1);
 this->ImageFromScannerTmp->SetExtent(0, newImageSize.width-1, 0, newImageSize.height-1, 0, 0 );
 this->ImageFromScannerTmp->SetNumberOfScalarComponents(3);
 this->ImageFromScannerTmp->SetScalarTypeToUnsignedChar();
 this->ImageFromScannerTmp->AllocateScalars();
 this->ImageFromScannerTmp->Update();
 */
 ...
 /*
 else if (this->makeImageFromScanner==0){
...
*/
 ...
 ///memcpy((void*)this->RGBImage->imageData, imageData->GetScalarPointer(), dsize);
 //memcpy((void*)this->VideoImageData->GetScalarPointer(), (void*)this->RGBImage->imageData, dsize);
 //memcpy((void*)this->RGBImage->imageData, imageData->GetScalarPointer(), vImageSize[0]*vImageSize[1]*3);
 //memcpy((void*)this->VideoImageData->GetScalarPointer(), (void*)this->RGBImage->imageData, dsize);
 
 ///memcpy((void*)this->ImageFromScanner->imageData, imageData->GetScalarPointer(), vImageSize[0]*vImageSize[1]*3);
 ///memcpy((void*)this->VideoImageData->GetScalarPointer(), (void*)this->ImageFromScanner->imageData, vImageSize[0]*vImageSize[1]*3);

///memcpy((void*)this->ImageFromScannerTmp->imageData, imageData->GetScalarPointer(), 256*256/*vImageSize[0]*vImageSize[1]*1*/);

///cvCvtColor( this->ImageFromScannerTmp, this->ImageFromScanner, CV_GRAY2RGB);       //comment not to undistort      at 10. 01. 07 - smkim
///memcpy((void*)this->VideoImageData->GetScalarPointer(), (void*)this->ImageFromScannerTmp->imageData, dsize/*vImageSize[0]*vImageSize[1]*3*/);


// memcpy((void*)this->VideoImageData->GetScalarPointer(), imageData->GetScalarPointer(), vImageSize[0]*vImageSize[1]*3);
