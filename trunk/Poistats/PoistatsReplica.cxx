...
PoistatsReplica::SpaceEvenly( ArrayPointer outputArray, const double floor, 
  ...
  for( int cOutputArray=0; cOutputArray<outputArray->size(); cOutputArray++ ) {
...
PoistatsReplica::CalculateCumulativeSum(
  ...
  ( *cumulativeSum )[ 0 ] = 0.0;
  for( int cRow=1; cRow<cumulativeSum->size(); cRow++ ) {
...
void PoistatsReplica::GetPerturbedBasePath( MatrixPointer perturbedPath, 
  ...
  for( int cRow=1; cRow<currentBasePath->rows()-1; cRow++ ) {
    for( int cColumn=0; cColumn<currentBasePath->cols(); cColumn++ ) {
...
PoistatsReplica::GenerateUnitSphereRandom( MatrixPointer randomUnitSphere ) {
  ...
  const int spatialDimensions = randomUnitSphere->columns();

  for( int cRow=0; cRow<randomUnitSphere->rows(); cRow++ ) {
    for( int cColumn=0; cColumn<randomUnitSphere->cols(); cColumn++ ) {
...
PoistatsReplica::GenerateConstrainedRandomPoint3D(
  ...
  if( isPointFound ) {
    ...
    for( int cColumn=0; cColumn<newRandomPoint->size(); cColumn++ ) {
...
bool PoistatsReplica::FindPointsWithinRadius(
  ...
  for( int cRow=0; cRow<points->rows(); cRow++ ) {
  ...
  for( int cRow=0; cRow<distances.size(); cRow++ ) {
...
PoistatsReplica::CopyPath( 
  ...
  for( int cRow = 0; cRow < source->rows(); cRow++ ) {
    for( int cColumn = 0; cColumn < source->cols(); cColumn++ ) {
