                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    mxClassID mtype;
    unsigned int axIdx, sdIdx, dwiAxIdx, dwiIdx, count; /* Loop iterators */
    Nrrd *nrrd;
    NrrdIoState *nio;
    airArray *mop;
    /* Meta data stuff */
    ...
    for (axIdx=0; axIdx<nrrd->dim; axIdx++) {
        for (sdIdx=0; sdIdx<nrrd->spaceDim; sdIdx++) {
            if( !mxIsNaN( nrrd->axis[axIdx].spaceDirection[sdIdx] ) ) {
    ...
    if (NULL != nrrd->kvp) { 
        /** find the index containing the gradient directions **/
        dwiAxIdx = -1;
        for (axIdx=0; axIdx<nrrd->dim; axIdx++) {
            if( nrrd->axis[axIdx].kind == 4 ) {
                dwiAxIdx = axIdx;
            }
        }
        if( dwiAxIdx == -1 ) {
            sprintf(errBuff, "%s: Unable to locate the gradient directions.\n%s", me, errPtr);
            airMopError(mop);
            mexErrMsgTxt(errBuff);
        }
        ...
        *bvalue_temp = bKVP;
        ...
        /** gradientdirections **/
        gradientdirections = mxCreateNumericMatrix( sizeI[dwiAxIdx], nrrd->spaceDim, mxDOUBLE_CLASS, mxREAL );
        gradientdirections_temp = (double *)mxGetData(gradientdirections);
        for (dwiIdx=0; dwiIdx<sizeI[dwiAxIdx]; dwiIdx++) {
            gradientdirections_temp[dwiIdx] = info[dwiIdx*nrrd->spaceDim];
            gradientdirections_temp[dwiIdx+sizeI[dwiAxIdx]] = info[dwiIdx*nrrd->spaceDim+1];
            gradientdirections_temp[dwiIdx+sizeI[dwiAxIdx]*2] = info[dwiIdx*nrrd->spaceDim+2];
        }
