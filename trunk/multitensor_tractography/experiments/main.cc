...
#include "../ukf_tractography/linalg.h"
#include "../ukf_tractography/signal_data.h"
#include "../ukf_tractography/utilities.h"
...
void CalculateError(const NrrdData* signal, const Nrrd* label_nrrd,
  ...
  for (size_t i = 0; i < lines.size(); ++i) {
    ...
    // other direction.
    vec_t start_dir = make_vec(0.0, 1.0, 0.0);
    ...
    for (size_t j = 0; j < lines[i].size(); ++j) {
      ...
      if (region != NONE) {
        ...
        if (region == HORIZONTAL) {
          m1_correct = make_vec(0.0, 1.0, 0.0);
          m2_correct = make_vec(0.0, 1.0, 0.0);
        } else if (region == CROSSING) {
          double y = sin(angle * M_PI / 180.0);
          double x = cos(angle * M_PI / 180.0);
          m1_correct = make_vec(0.0, 1.0, 0.0);
          m2_correct = make_vec(0.0, x, y);
...
//          m2_correct = make_vec(0.0, y, x);
        ...
        if (calc_fa_error) {
          ...
          if (state.size() == 5 || state.size() == 10) {
            ...
            fa = l2fa(state[3], state[4], state[5]);
          } else {
            assert(true);
          }
          ...
          if (debug_output) {
            std::cout << "FA (FA, correct FA, error): " << fa << ", "
                      ...
                      << std::endl;
            if (state.size() == 5 || state.size() == 10) {
              std::cout << "EVs: " << state[3] << ", " << state[4] << std::endl;
            } else {
              std::cout << "EVs: " << state[3] << ", " << state[4] << ", "
                        << state[5] << std::endl;
            }
          }
        }
  ...
  double ang_err_non_cross_sd = 0.0;
  if (angular_error.size()) {
    ...
    for (size_t i = 0; i < angular_error.size(); ++i) {
      ang_err_non_cross_sd += (angular_error[i] - ang_err_non_cross) *
                              (angular_error[i] - ang_err_non_cross);
    }
    ang_err_non_cross_sd /= angular_error.size();
    ang_err_non_cross_sd = sqrt(ang_err_non_cross_sd);
  } else {
    ang_err_non_cross = -1.0;
    ang_err_non_cross_sd = -1.0;
  }
  ...
  if (angular_error_crossing.size()) {
    ...
    for (size_t i = 0; i < angular_error_crossing.size(); ++i) {
      ang_err_cross_sd += (angular_error_crossing[i] - ang_err_cross) *
                          (angular_error_crossing[i] - ang_err_cross);
    }
    ang_err_cross_sd /= angular_error_crossing.size();
    ang_err_cross_sd = sqrt(ang_err_cross_sd);
  } else {
  ...
  if (calc_fa_error) {
    ...
    for (size_t i = 0; i < fa_error.size(); ++i) {
      fa_error_sd += (fa_error[i] - fa_error_avg) *
                     (fa_error[i] - fa_error_avg);
    }
    fa_error_sd /= fa_error.size();
    fa_error_sd = sqrt(fa_error_sd);
  }
  if (debug_output) {
    std::cout << std::endl;
  }
  ...
  std::cout << ang_err_non_cross * 180.0 / M_PI << "\t"
            << ang_err_non_cross_sd * 180.0 / M_PI << "\t"
            << ang_err_cross * 180.0 / M_PI << "\t"
            << ang_err_cross_sd * 180.0 / M_PI << "\t"
            << nmse_avg;
  if (calc_fa_error) {
    std::cout << "\t" << fa_error_avg << "\t" << fa_error_sd;
  }
  std::cout << std::endl;
}
