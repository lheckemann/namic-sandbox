...
//============================================================
//
// The following two classes are just wrappers around
// stream output methods. They are here in order to
// avoid duplicating the code of MGHImageIO::WriteHeader
// and MGHImageIO::WriteData
//
// The pattern chosen for this is Policy, as
// virtual template functions are not supported.
//
//============================================================

//---------------------------------------
//
// Wraps the STL output stream operations
//
//---------------------------------------
...
public:
    ...
    virtual void WriteBuffer(const char* buffer,
                             unsigned int size)
    {
...
//-------------------------------------
//
// Wraps the GZ output stream operations
//
//-------------------------------------
class GZWrapper
{
...
public:
    ...
    long  m_len;
    int   m_tag;
    TagReader(gzFile iFile)
            : m_fp(iFile), m_data(NULL),
            m_len(0), m_tag(-1)
    {}
    ~TagReader()
    {
    ...
    {
        TReadZ(m_fp, m_tag);
        if ( gzeof(m_fp) ) return false;

        // it is annoying the format is not uniform...
        // but maybe that's why these guys are called.... old
        //
        switch (m_tag)
        {
        case fs::TAG_OLD_SURF_GEOM:
        case fs::TAG_OLD_USEREALRAS:
        case fs::TAG_OLD_COLORTABLE:
            m_len = 0; break;
        case fs::TAG_OLD_MGH_XFORM:
            TReadZ(m_fp, m_len);
            m_len -= 1; // doesn't include NULL
        default:
            TReadZ(m_fp, m_len);
        }
          ...
          std::string contents)
{
    ...
    itk::ByteSwapper<int>::SwapFromSystemToBigEndian(&tag);
    oss.write( (const char*)(&tag),
               sizeof(int) );

    long len = 1+contents.size();
    long l2 = len;

    itk::ByteSwapper<long>::SwapFromSystemToBigEndian(&l2);
    oss.write( (const char*)(&l2),
               sizeof(long) );
...
MGHImageIO::ReadVolumeHeader(gzFile fp)
{
    ...
    if ( TReadZ( fp, fBuf) )
    {
        ...
        EncapsulateMetaData<float>(thisDic,
                                   std::string("TR"),
                                   fBuf);
    ...
    //
    MetaDataDictionary& thisDict
    = this->GetMetaDataDictionary();
    TagReader tagReader(fp);
    while ( tagReader.Read() )
    {
        std::cout << " read tag " << tagReader.m_tag << std::endl;
        switch (tagReader.m_tag)
        {
        case fs::TAG_OLD_MGH_XFORM:
        case fs::TAG_MGH_XFORM:
            // store the information - usually a filename
            {
                std::string strFileName(tagReader.m_data);
                EncapsulateMetaData(thisDict,
                                    "XForm",
                                    strFileName);
            }
            break;
        case fs::TAG_CMDLINE:
            // see if a previous command was there
            {
                std::vector<std::string> vstr;
                ExposeMetaData( thisDict,
                                "CmdLines",
                                vstr );

                vstr.push_back( std::string( tagReader.m_data ) );
            ...
            break;
        case fs::TAG_AUTO_ALIGN:
            {
                std::cout << " got here\n";
                std::vector<double> vdata;
                double dbuf;
                std::istringstream iss(tagReader.m_data);
                // the first string is AutoAlign - why?
                std::string strDummy;
                iss >> strDummy;
                // read a 4x4 matrix - fastest component is the column
                for (unsigned int ui=0; ui<16; ++ui)
                {
                    iss >> dbuf;
                    vdata.push_back(dbuf);
                }
                EncapsulateMetaData(thisDict,
                                    "AutoAlign",
                                    vdata);
...
MGHImageIO::GetComponentSize() const
{
    ...
    {
    case UCHAR:
        returnValue = sizeof(unsigned char); break;
    case SHORT:
        returnValue = sizeof(short); break;
    case INT:
        returnValue = sizeof(int); break;
    case FLOAT:
        returnValue = sizeof(float); break;
