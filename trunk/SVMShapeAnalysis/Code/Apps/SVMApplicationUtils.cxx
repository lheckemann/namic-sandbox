...
const SVMApplicationUtils::ClassLabelType 
SVMApplicationUtils::Class1Label = 0;

const SVMApplicationUtils::ClassLabelType
SVMApplicationUtils::Class2Label = 1;

const MET_ValueEnumType
SVMApplicationUtils::METAValueType = MET_FLOAT;

SVMApplicationUtils::OutputLevel SVMApplicationUtils::outputLevel = 
SVMApplicationUtils::Standard;
...
template <class T>
void
GetDataFromMETA(SVMApplicationUtils::VectorReaderType::Pointer& vectorReader,
                itk::VariableLengthVector<T>& mv)
{
  vectorReader->
    GetOutput(SVMApplicationUtils::METAValueType, &mv, true); 
}
...
GetDataFromMETA(SVMApplicationUtils::VectorReaderType::Pointer& vectorReader,
                itk::Array<T>& mv)
{
  vectorReader->GetOutput(SVMApplicationUtils::METAValueType, &mv, true);  
}
...
GetDataFromMETA(SVMApplicationUtils::VectorReaderType::Pointer& vectorReader,
                itk::Vector<T, I>& mv)
{
  vectorReader->GetOutput(SVMApplicationUtils::METAValueType, &mv);  
}
                      ...
                      MembershipSampleType::Pointer membershipSample)
{
       ...
       ++iter)
  {
    ...
    SVMApplicationUtils::LoadMeasurementVectorFromVectorFile(*iter, mv);
...
ExtractFilename(const std::string& fullyQualifiedFilename)
{
  ...
  std::string::size_type pos = fullyQualifiedFilename.find_last_of("/\\");
  ...
  {
    return fullyQualifiedFilename.substr(pos+1);
  }
}
