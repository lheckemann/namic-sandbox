    ...
    for (int i = 0; i < m_N; ++i )
      {
        ...
        TVnlVector vectorForThisImage = itkImageToVnlVector<double, m_imageDimension>(thisImage);
...
//         //debug//
    ...
    m_meanImage = vnlVectorToItkImage<TData, m_imageDimension>(vctMean, m_imageSize);

    // copy information from input images
    m_meanImage->CopyInformation(m_imageList[0]);
    ...
    for (int i = 0; i < m_N; ++i )
      {
        ...
        typename TItkImage::Pointer eigenImg = vnlVectorToItkImage<TData, 3>(eigenImgVec, m_imageSize);

        // copy information from input images
        eigenImg->CopyInformation(m_imageList[0]);
