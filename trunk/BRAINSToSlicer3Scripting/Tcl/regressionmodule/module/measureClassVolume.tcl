...
proc measureClassVolume { pathToRegressionDir dateString } {
        ...
        set ModuleDescription "Testing of b2 measure class-volume roi mask"
...
##ReportTestStatus $LogFile  [ expr { [ b2 destroy every gtsurface ] != -1 } ] $ModuleName "Destroying every gtsurface"
    ...
    set SubTestDes "Load requisite objects for volume creation - cranRoi"
      set cranRoi [b2 convert mask to roi Coronal $brainMask  ]
...
set output1 [ b2 measure class-volume mask $brainMask $TestImageID]
CoreMeasuresEpsilonTest "volume mask" 0.0001 $KnownValues $output1 $LogFile $ModuleName

set KnownValues2 {
{Grey_Matter_CubicCentimeters 778.069473274558163} {White_Matter_CubicCentimeters 388.632925347560615} {CSF_CubicCentimeters 295.586888945129090} {Blood_CubicCentimeters 18.646432399749756} {No_Class_CubicCentimeters 1.368180274963379}
}
set output2 [ b2 measure class-volume roi $cranRoi $TestImageID]
...
set output3 [ b2 measure class-volume roi $cranRoi $TestImageID min= 100 max= 102 ]
...
set output4 [ b2 get image value $TestImageID 100 100 100 ]
...
set output5 [ b2 get image value $TestImageID 100 100 99 ]
...
set output6 [ b2 get image value $TestImageID 100 100 99.5 ]
...
set output7 [ b2 measure roi perimeter $cranRoi 102 ]
...
set output8 [ b2 measure roi perimeter $cranRoi -1 ]
CoreMeasuresEpsilonTest "measure roi perimeter for whole roi" 0.0001 $KnownValues8 $output8 $LogFile $ModuleName
set SubTestDes "Converting slice '-1' to the label ALL in 'b2 measure roi perimeter' output"
...
set output9 [ b2 measure bounds mask $brainMask ]
...
set output10 [ b2 measure resel-size mask $brainMask $TestImageID ]
...
# Clean up Files
    ...
    ReportTestStatus $LogFile  [ expr { [ b2 destroy roi $cranRoi ] != -1 } ] $ModuleName "Destroying roi $cranRoi"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask [lindex [lindex $output2 5] 1] ] != -1 } ] $ModuleName "Destroying mask 1"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask [lindex [lindex $output3 5] 1] ] != -1 } ] $ModuleName "Destroying mask 2"
