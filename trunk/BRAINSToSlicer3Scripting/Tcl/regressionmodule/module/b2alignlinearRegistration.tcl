...
#   set LogFile stderr
    ...
    set im1 [b2 load image "${pathToRegressionDir}/SGI/MR/5x-B2/TEST/31_003/148313.003.002" data-type= signed-16bit]
    ...
    set num_ret [b2 load transform "${OUTPUT_DIR}/b2alignlinear.test.air16"]
    ...
    b2 set transform ${num_ret} image ${im1}
        set im2 [b2 load image "${pathToRegressionDir}/SGI/MR/5x-B2/TEST/31_003/148313.003.002" data-type= signed-16bit]
        set xfm [b2 load transform "${pathToRegressionDir}/SGI/MR/5x-B2/TEST/10_ACPC/b2alignlinear.reference.air16"]
    b2 set transform ${xfm} image ${im2}
    set im3 [b2 subtract images [list ${im1} ${im2}]]
    set errMask [b2 threshold image ${im3} 1 absolute-value= True ]
