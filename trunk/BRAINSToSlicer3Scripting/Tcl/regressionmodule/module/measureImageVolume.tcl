...
proc measureImageVolume { pathToRegressionDir dateString } {
        ...
        set ModuleDescription "Testing of b2 measure image roi mask"
...
##ReportTestStatus $LogFile  [ expr { [ b2 destroy every gtsurface ] != -1 } ] $ModuleName "Destroying every gtsurface"
    ...
    set SubTestDes "Load requisite objects for volume creation - hemRoi"
      set hemRoi [b2 convert mask to roi Coronal $brainMask  ]
...
#######################################################################################################################
        ...
        set output2 [ b2 measure image roi $hemRoi $TestImageID]
          set standards {
{Mean 135.789818910665275} {Mean-Absolute-Deviation 57.342890663550520} {Variance 4419.842171388293536} {Standard-Deviation 66.481893560489794} {Skewness 1.327814335331516} {Kurtosis -1.087230908271656} {Minimum 0.000000000000000} {Maximum 250.000000000000000}
    }
          CoreMeasuresEpsilonTest "2. volume roi" 0.0000001 $standards $output2  $LogFile $ModuleName

        set output3 [ b2 measure image roi $hemRoi $TestImageID min= 100 max= 102 ]
          set standards {
...
#######################################################################################################################
#######################################################################################################################
        set output4 [ b2 measure image mask $brainMask $TestImageID functional-flag= true]
          set standards {
...
#######################################################################################################################
        ...
        set output5 [ b2 measure image roi $hemRoi $TestImageID functional-flag= true]
          set standards {
{Mean 135.915269967905346} {Mean-Absolute-Deviation 57.277133636247079} {Variance 4406.874772253224364} {Standard-Deviation 66.384296126819208} {Skewness 1.326921776548464} {Kurtosis -1.090300604489888} {Minimum 1.000000000000000} {Maximum 250.000000000000000}
    }
          CoreMeasuresEpsilonTest "5. volume roi" 0.0000001 $standards $output5  $LogFile $ModuleName

        set output6 [ b2 measure image roi $hemRoi $TestImageID min= 100 max= 102 functional-flag= true ]
          set standards {
...
# Clean up Files
    ...
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $ValueArray2(MaskHandle) ] != -1 } ] $ModuleName "Destroying mask $ValueArray2(MaskHandle)"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $ValueArray3(MaskHandle) ] != -1 } ] $ModuleName "Destroying mask $ValueArray3(MaskHandle)"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $ValueArray5(MaskHandle) ] != -1 } ] $ModuleName "Destroying mask $ValueArray5(MaskHandle)"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $ValueArray6(MaskHandle) ] != -1 } ] $ModuleName "Destroying mask $ValueArray6(MaskHandle)"
