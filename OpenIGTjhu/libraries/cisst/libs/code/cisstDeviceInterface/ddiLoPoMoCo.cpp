...
CMN_IMPLEMENT_SERVICES(ddiLoPoMoCo);


ddiLoPoMoCo::ddiLoPoMoCo(const char* deviceName):
...
void ddiLoPoMoCo::Configure(const char *filename) {
  ...
  return;
    ...
    CMN_LOG_CLASS(3) << "Configuring LoPoMoCo with \"" << filename << "\"" << std::endl;
  ...
  xmlConfig.GetXMLValue(context.c_str(), path, VoltageToCounts[axis]);
    ...
    CMN_LOG_CLASS(3) << "Configured LoPoMoCo" << std::endl;
}
