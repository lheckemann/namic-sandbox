...
// $Id: ddiFourLoPoMoCo.cpp,v 1.5 2006/05/13 21:29:21 kapoor Exp $
...
CMN_IMPLEMENT_SERVICES(ddiFourLoPoMoCo);


ddiFourLoPoMoCo::ddiFourLoPoMoCo(const char* deviceName,
         const char* component1Name,
         const char* component2Name,
         const char* component3Name,
         const char* component4Name,
    ...
    ddiCompositeDevice(deviceName)
{
    ...
    ddiDeviceInterface *dev3 = new ddiLoPoMoCo(component3Name);
    ddiCompositeDevice::AddDevice(dev3);
    ddiDeviceInterface *dev4 = new ddiLoPoMoCo(component4Name);
    ddiCompositeDevice::AddDevice(dev4);
    strncpy(RelativePathToConfigFiles, relativePathToConfigFiles, 255);
}


ddiFourLoPoMoCo::~ddiFourLoPoMoCo()
{
}


void ddiFourLoPoMoCo::Configure(const char* filename)
{
    CMN_LOG_CLASS(3) << "Configuring ddiFourLoPoMoCo" << std::endl;

    // if no file name has been provided, use defaults
    if (!filename) {
  CMN_LOG_CLASS(2) << "Configuring ddiFourLoPoMoCo with default LoPoMoCo{1,2,3,4}.xml"
       <<std::endl;
  (ddiCompositeDevice::GetDevice(0))->Configure("LoPoMoCo1.xml");
  (ddiCompositeDevice::GetDevice(1))->Configure("LoPoMoCo2.xml");
  (ddiCompositeDevice::GetDevice(2))->Configure("LoPoMoCo3.xml");
  (ddiCompositeDevice::GetDevice(3))->Configure("LoPoMoCo4.xml");
  return;
    }

    // load configuration file for ddiFourLoPoMoCo
    CMN_LOG_CLASS(3) << "Configuring ddiFourLoPoMoCo with file: " << filename << std::endl;
  ...
  CMN_LOG_CLASS(1) << "Couldn't find all the required ConfigFile in " << filename << std::endl;
    ...
    CMN_LOG_CLASS(3) << "Configuring ddiFourLoPoMoCo finished" << std::endl;
}
