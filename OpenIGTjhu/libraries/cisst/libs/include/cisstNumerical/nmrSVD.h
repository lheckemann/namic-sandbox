...
/*
  $Id: nmrSVD.h,v 1.22 2006/01/27 00:59:49 anton Exp $
...
public:
    ...
    */
    ...
    {
        MatrixSizeType matrixSize(A.rows(), A.cols());
        return matrixSize;
    }
...
public:
    ...
    enum {MIN_MN = ((size_type)M < (size_type)N) ? (size_type)M : (size_type)N};
    enum {LWORK_1 = 3 * (size_type)MIN_MN + ((size_type)M > (size_type)N) ? (size_type)M : (size_type)N};
    enum {LWORK_2 = 5 * MIN_MN};
    enum {LWORK = ((unsigned int)LWORK_1 > (unsigned int)LWORK_2) ? (unsigned int)LWORK_1 : (unsigned int)LWORK_2};
#endif // DOXYGEN
...
#elif CISST_HAS_CISSTNETLIB
    ftnlen jobu_len = (ftnlen)1, jobvt_len = (ftnlen)1;
    la_dzlapack_MP_sgesvd_nat__(&jobu, &jobvt, &ldu, &ldvt,
                                A.Pointer(), &lda, S.Pointer(),
                                UPtr, &ldu,
                                VtPtr, &ldvt,
                                workspace.Pointer(), &lwork, &info,
