  ...
  while (!input.eof()) {
    ...
    if (!s.compare("DATASET")) {
      ...
      for (int i = 0; i < num_fields; ++i) {
        ...
        input >> tmp;
        assert(tmp == num_points);
        _fields[name].resize(num_points);
        input >> s;
        assert(!s.compare("float"));

        for (int i = 0; i < num_points; ++i) {
          _fields[name][i].resize(dim);

          for (int j = 0; j < dim; ++j) {
            input >> _fields[name][i][j];
          }
        }
...
//  }
  ...
  for (cit = _fields.begin(); cit != _fields.end(); ++cit) {

    std::cout << cit->first << " (" << cit->second.size() << "):" << std::endl;
    for (size_t j = 0; j < cit->second.size(); ++j) {
      std::cout << cit->second[j][0];
      for (size_t k = 1; k < cit->second[j].size(); ++k) {
        std::cout << " " << cit->second[j][k];
      }
      std::cout << std::endl;
    }
  }
