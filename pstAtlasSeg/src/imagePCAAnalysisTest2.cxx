...
int main( int argc, char * argv[] )
{
  ...
  std::vector< std::string > listOfLines = newProstate::readTextLineToListOfString<char>(fileContainingFileNames);
  ...
  std::vector<imagePointer_t> inputImageSeries = newProstate::readImageSeries3<pixel_t>(listOfLines);
  ...
  typedef newProstate::CImagePCAAnalysis3D<pixel_t> CImagePCAAnalysis3D_t;
  ...
  newProstate::writeImage3< double >( meanImg, "meanImage.nrrd" );


  // output eigen
  char name[500];
  sprintf(name, "eigenImg_%d.nrrd", ith);
  newProstate::writeImage3< pixel_t >( eigenImg, name );
