...
vtkDebugLeaks::ObjectAllocationInfoType* vtkDebugLeaks::ObjectAllocationInfo=new ObjectAllocationInfoType;
...
//----------------------------------------------------------------------------
void vtkDebugLeaks::DestructObject(vtkObject *ptr)
{
  ObjectAllocationInfo->erase(ptr);
}
...
int vtkDebugLeaks::PrintCurrentLeaks()
{
  ...
  // Print stack trace for leaked objects
  // Note that printing may cause crash as some referenced objects may have been already deleted.
  // Instead, it is recommended to attach a debugger to analyze the leaked objects.
  bool enableLeakedObjectPrinting=false; 
  for (ObjectAllocationInfoType::iterator it=ObjectAllocationInfo->begin(); it!=ObjectAllocationInfo->end(); ++it)
  {
    vtkObject* leakedObject=(*it);
    std::cout << "Leaked object: " << leakedObject->GetClassName() << "(" << leakedObject << ")" << std::endl;
    if (enableLeakedObjectPrinting)
    {
      std::cout << "------------------------------------" << std::endl;
      leakedObject->Print(std::cout);
      std::cout << "------------------------------------" << std::endl;
    }
  }
