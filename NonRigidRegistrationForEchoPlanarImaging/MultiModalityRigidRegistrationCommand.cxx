...
int main( int argc, char *argv[] )
{
  if( argc < 5 )
    ...
    std::cerr << " outputTransformile  ";
    std::cerr << " [resolutionLevels{1,2,3}]";
    std::cerr << " [qualityLevels{1,2}]";
    std::cerr << " [fixedImageMask]";
  ...
  const char * inputTransformFileName  = argv[3];
  ...
  */
  ...
  if( argc > 7 )
    {
    fixedMaskReader->SetFileName( argv[7] );
    try
      {
  ...
  if( argc > 8 )
    {
    movingMaskReader->SetFileName( argv[8] );
    try
      {
