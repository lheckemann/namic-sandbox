...
template< class TInputDTIImage, class TInputb0Image, class TInputDWIImage, class TInputWhiteMatterProbabilityImage,
  class TOutputConnectivityImage >
StochasticTractographyFilter< TInputDTIImage, TInputb0Image, TInputDWIImage, TInputWhiteMatterProbabilityImage,
...
StochasticTractographyFilter< TInputDTIImage, TInputb0Image, TInputDWIImage, TInputWhiteMatterProbabilityImage,
  TOutputConnectivityImage >
::CalculateConstrainedModelParameters( const DTIImageType::PixelType& dtipixel,
    const b0ImageType::PixelType& b0pixel,
...
::CalculateResidualVariance( const DWIVectorImageType::PixelType& noisydwi,
  ...
  for(unsigned int i=0; i<N; i++)
    residualvariance+=vnl_math_sqr((vcl_log(noisydwi[i]/noisefreedwi[i])));
...
StochasticTractographyFilter< TInputDTIImage, TInputb0Image, TInputDWIImage, TInputWhiteMatterProbabilityImage,
  TOutputConnectivityImage >
::CalculateLikelihood( const DTIImageType::PixelType& dtipixel,
  const b0ImageType::PixelType& b0pixel,
  const DWIVectorImageType::PixelType &dwipixel, 
  ...
  CalculateConstrainedModelParameters( dtipixel, b0pixel, constrainedparams );
  CalculateNoiseFreeDWIFromConstrainedModel( constrainedparams, noisefreedwi );
  CalculateResidualVariance( dwipixel, noisefreedwi, 6, residualvariance );
