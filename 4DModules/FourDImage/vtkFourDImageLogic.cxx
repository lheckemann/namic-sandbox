...
//---------------------------------------------------------------------------
int vtkFourDImageLogic::RunSeriesCropping(const char* inputBundleNodeID,
                                             int imin, int imax, int jmin, int jmax, int kmin, int kmax)
{
  ...
  if (!inputBundleNode)
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesCropping(): Irregular input bundle" << std::endl;
    return 0;
    }
  ...
  if (imin < 0 || imin >= imax || jmin < 0 || jmin >= jmax || jmin < 0 || jmin >= jmax)
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesCropping(): irregular index" << std::endl;
    return 0;
    }
  ...
  for (int i = 0; i < nFrames; i ++)
    ...
    vtkFourDImageLogic::RunCropping(volumeNode, volumeNode,
  ...
  vtkFourDImageLogic::RunCropping(dinode0, dinode0, imin, imax, jmin, jmax,kmin, kmax);
  vtkFourDImageLogic::RunCropping(dinode1, dinode1, imin, imax, jmin, jmax,kmin, kmax);
...
//---------------------------------------------------------------------------
int vtkFourDImageLogic::RunSeriesRegistration(int sIndex, int eIndex, int kIndex, 
                                                 ...
                                                 RegistrationParametersType& deformableParam)
{
  ...
  if (!inputBundleNode)
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesRegistration(): Irregular input bundle" << std::endl;
    return 0;
    }
  if (sIndex < 0 ||
    ...
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesRegistration(): irregular index" << std::endl;
    return 0;
    }
  ...
  if (!outputBundleNode)
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesRegistration(): Irregular output bundle" << std::endl;
    return 0;
    }
  ...
  if (!fixedNode)
    {
    std::cerr << "int vtkFourDImageGUI::RunSeriesRegistration(): no fixed frame node ID found." << std::endl;
    }
  ...
  transformNode->SetDescription("Created by FourDImage module");
...
//---------------------------------------------------------------------------
int vtkFourDImageLogic::RunCropping(vtkMRMLScalarVolumeNode* inputNode,
                                       vtkMRMLScalarVolumeNode* outputNode,
                                       int imin, int imax, int jmin, int jmax, int kmin, int kmax)
{
...
//---------------------------------------------------------------------------
int vtkFourDImageLogic::RunAffineRegistration(vtkMRML4DBundleNode* bundleNode,
                                                 vtkMRMLScalarVolumeNode* fixedNode,
                                                 vtkMRMLScalarVolumeNode* movingNode,
...
//---------------------------------------------------------------------------
int vtkFourDImageLogic::RunDeformableRegistration(vtkMRML4DBundleNode* bundleNode,
