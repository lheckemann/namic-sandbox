...
SET (LIBRARY_OUTPUT_PATH ${VTKPACS_BINARY_DIR}/bin/ CACHE PATH 
     "Single output directory for building all libraries.")

SET (EXECUTABLE_OUTPUT_PATH ${VTKPACS_BINARY_DIR}/bin/ CACHE PATH 
...
IF (USE_VTK_FILE)
  ...
  SET(VTKPACS_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS} CACHE INTERNAL 
      "Is this VTKPACS built with shared libraries.")
...
IF (VTK_WRAP_TCL)
  ...
  IF (VTKPACS_WRAP_TCL)
    ...
    ELSE (NOT VTK_WRAP_TCL_EXE)
      ...
      FIND_FILE (VTK_WRAP_HINTS hints ${VTKPACS_SOURCE_DIR}/Wrapping)
  ...
  ENDIF (VTKPACS_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (VTKPACS_WRAP_TCL)
    MESSAGE("Warning. VTKPACS_WRAP_TCL is ON but the VTK version you have chosen has not support for Tcl (VTK_WRAP_TCL is OFF). Please set VTKPACS_WRAP_TCL to OFF.")
    SET (VTKPACS_WRAP_TCL OFF)
  ENDIF (VTKPACS_WRAP_TCL)
...
IF (VTK_WRAP_PYTHON)
  ...
  ENDIF (VTKPACS_WRAP_PYTHON)

ELSE (VTK_WRAP_PYTHON)

  IF (VTKPACS_WRAP_PYTHON)
    MESSAGE("Warning. VTKPACS_WRAP_PYTHON is ON but the VTK version you have chosen has not support for Python (VTK_WRAP_PYTHON is OFF). Please set VTKPACS_WRAP_PYTHON to OFF.")
    SET (VTKPACS_WRAP_PYTHON OFF)
  ENDIF (VTKPACS_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  IF (VTKPACS_WRAP_JAVA)
    ...
    IF (VTK_WRAP_JAVA_EXE AND VTK_PARSE_JAVA_EXE)
      ...
      IF (NOT VTK_JAVA_HOME)
        SET (VTK_JAVA_HOME ${VTKPACS_BINARY_DIR}/java/vtk CACHE PATH "Path to Java install")
  ...
  ENDIF (VTKPACS_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (VTKPACS_WRAP_JAVA)
    MESSAGE("Warning. VTKPACS_WRAP_JAVA is ON but the VTK version you have chosen has not support for Java (VTK_WRAP_JAVA is OFF). Please set VTKPACS_WRAP_JAVA to OFF.")
    SET (VTKPACS_WRAP_JAVA OFF)
  ENDIF (VTKPACS_WRAP_JAVA)
