...
//        fprintf(stderr, " <--\n");
  ...
  ierr = MatCreateMPIAIJ(PETSC_COMM_WORLD,
    ...
    */
    ...
    /*
    if (myid==0) 
      fprintf(stderr,"Now solving the system ... ");
    */
    
    //ierr = SLESCreate(PETSC_COMM_WORLD,&sles); CHKERRQ(ierr);
    KSPCreate(PETSC_COMM_WORLD, &ksp);CHKERRQ(ierr);
    //ierr = SLESSetOperators(sles,K_global,K_global,DIFFERENT_NONZERO_PATTERN); 
    ...
    //ierr = SLESSolve(sles,F_global,u_displ,&its); CHKERRQ(ierr);   
...
void PETScDeformWrapper::ComputeShape(float vec[4][3], float a[4], float b[4], float c[4],
  ...
  for (l=0;l<4;l++){
    for (k=1;k<3;k++) vec_tmp[l][k]=vec[l][k]; 
    vec_tmp[l][0]=1;
  }
  for (j=0;j<4;j++)
    b[j]=-calc_det_3(vec_tmp[(j+1)%4],vec_tmp[(j+2)%4],vec_tmp[(j+3)%4]);
    
  /* c : second column filled with ones*/
  for(l=0;l<4;l++){
    for (k=0;k<3;k++) vec_tmp[l][k]=vec[l][k]; 
    vec_tmp[l][1]=1;
  }
  for (j=0;j<4;j++)
    c[j]=-calc_det_3(vec_tmp[(j+1)%4],vec_tmp[(j+2)%4],vec_tmp[(j+3)%4]);

  /* d: third column filled with ones*/
  for(l=0;l<4;l++){
    for (k=0;k<2;k++) vec_tmp[l][k]=vec[l][k]; 
    vec_tmp[l][2]=1;
  }
  for (j=0;j<4;j++)
    d[j]=-calc_det_3(vec_tmp[(j+1)%4],vec_tmp[(j+2)%4],vec_tmp[(j+3)%4]);
