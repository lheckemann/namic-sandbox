...
int main( int argc, char *argv[] )
{
  if( argc < 5 || (argc < 4+atoi(argv[4])))
  {
    ...
    std::cerr << " SeedsFile4";
    std::cerr << " ... ";
    std::cerr << " ROI <1 femur> <2 tibia> <3 patella> " ;
    std::cerr << std::endl;
    return EXIT_FAILURE;
  }
...
findClosestClusterCenterIndex(float x, float y, float *xClusterCenter, float *yClusterCenter, float *xStandardDev, float *yStandardDev, int numberOfClusters, float *xnumberOfSds, float *ynumberOfSds)
{
 ...
 for(i=0; i<numberOfClusters; i++)
 {
  ...
  else if(i == 3) // case for fat
  {
   if(x>=(xClusterCenter[i-2]+xnumberOfSds[i-2]*xStandardDev[i-2]) && x<=(xClusterCenter[i]+xnumberOfSds[i]*xStandardDev[i]) && y>=(yClusterCenter[i]-ynumberOfSds[i]*yStandardDev[i]) && y<=(yClusterCenter[i]+ynumberOfSds[i]*yStandardDev[i]) && flag==0)
   {
    minIndex=i;
    flag=1;
   }

   else if(x>=(xClusterCenter[i-2]+xnumberOfSds[i-2]*xStandardDev[i-2]) && x<=(xClusterCenter[i]+xnumberOfSds[i]*xStandardDev[i]) && y>=(yClusterCenter[i]-ynumberOfSds[i]*yStandardDev[i]) && y<=(yClusterCenter[i]+ynumberOfSds[i]*yStandardDev[i]) && flag==1)
   {
    return 21;
   }
   continue;
