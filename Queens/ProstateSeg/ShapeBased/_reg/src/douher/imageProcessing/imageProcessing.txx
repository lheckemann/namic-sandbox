...
namespace douher
{
  ...
  void
  resetCornerAndOrigin(typename TItkImage::Pointer img)
  {
    /*
      corner := img->GetLargestPossibleRegion().GetIndex();
             == iterWithIndex.GoToBegin(); iterWithIndex.GetIndex();

      origin := space origion, affect display, physical coordinate, but doesn't not affect IJK coordinate
             == img->GetOrigin();

       If the corner idx is not [0, 0, 0], accumulate it to space
       origin, and clear it to [0, 0, 0]
    */
    unsigned int dim = TItkImage::GetImageDimension();

    typename TItkImage::RegionType region = img->GetLargestPossibleRegion();
    typename TItkImage::IndexType corner = region.GetIndex();
    typename TItkImage::PointType spaceOrigin = img->GetOrigin();
    for (unsigned int i = 0; i < dim; ++i)
      {
        spaceOrigin[i] += corner[i];
        corner[i] = 0;
      }
  ...
  */
  ...
  /* affineTransform, origin is NOT center of image as the above ones */
  ...
  affineTransform(typename itk::Image< TPixel, dim >::Pointer image,    \
                  ...
                  const vnl_vector< double >& translation,              \
                  TPixel fillValue,                                     \
                  char interpType, bool relative)
  {
    // when size of the transformed image is the same
    typename itk::Image< TPixel, dim >::SizeType newSize = image->GetLargestPossibleRegion().GetSize();
    
    return affineTransform<TPixel, dim>(image, A, translation, fillValue, interpType, newSize, relative);
  }


  /* affineTransform, origin is NOT center of image as the version above */
  template< typename TPixel, unsigned int dim > 
  typename itk::Image< TPixel, dim >::Pointer 
  affineTransform(typename itk::Image< TPixel, dim >::Pointer image, \
                  const vnl_matrix< double >& A,                        \
                  ...
                  TPixel fillValue,                                     \
                  char interpType,                                      \
                  typename itk::Image< TPixel, dim >::SizeType newSize, bool relative)
  {
    ...
    /* allocate the new image */
    TImagePointer newImage = TImage::New();
    ...
    /*
      ...
      f[ A*(x, y)^T + (transX, transY)^T ]
    */

    /* continuous idx is used to extract between-grid values*/
    typedef itk::ContinuousIndex<double, dim> TContinuousIndex;

    /* set up the for the old image */
    typename itk::InterpolateImageFunction<TImage, double>::Pointer interpolator;
    ...
    */
    {
      typedef itk::ImageRegionIteratorWithIndex<TImage> IteratorWithIdxType;
      IteratorWithIdxType newImageIter( newImage, newImage->GetLargestPossibleRegion() );
      vnl_vector< double > contNewIdx(dim);
      typename TImage::IndexType newIdx;
      TContinuousIndex contNewIdxMapBack;
      
      typename TImage::IndexType cornerIdx = newImage->GetLargestPossibleRegion().GetIndex();
      
      for (newImageIter.GoToBegin(); !newImageIter.IsAtEnd(); ++newImageIter)
        {
          newIdx = newImageIter.GetIndex();

          if (relative)
            {
              for (unsigned int i = 0; i < dim; ++i)
                {
                  newIdx[i] -= cornerIdx[i];
                }
            }
          ...
          // transform
          contNewIdx = A*contNewIdx + translation;
          ...
          if (relative)
            {
              for (unsigned int i = 0; i < dim; ++i)
                {
                  contNewIdxMapBack[i] += cornerIdx[i];
                }
            }

          if (interpolator->IsInsideBuffer(contNewIdxMapBack))
            {
              //newImageIter.Set( static_cast<TPixel>(round(interpolator->EvaluateAtContinuousIndex( contNewIdxMapBack ) ) ) ); 
              // good for binary img

              newImageIter.Set( static_cast<TPixel>(interpolator->EvaluateAtContinuousIndex( contNewIdxMapBack ) ) );
            }
        }
    }
  ...
  typename itk::Image<TPixel, 3>::Pointer constPadImage(typename itk::Image<TPixel, 3>::Pointer img, TPixel padValue, \
                                                        ...
                                                        long zHeadNum, long zTailNum)
  {
    /* 
       1. convert to carray
       2. use const pad for carray
       3. convert back to image
    */

    // 1.
    typename cArray3D<TPixel>::Pointer array = itkImageToArray3< TPixel >(img);

    // 2.
    typename cArray3D<TPixel>::Pointer paddedArray = constPadArray<TPixel>( array, padValue, xHeadNum, xTailNum, yHeadNum, yTailNum, zHeadNum, zTailNum);

    // 3.
    typename itk::Image<TPixel, 3>::Pointer paddedImg = cArray3ToItkImage<TPixel>(paddedArray);

    paddedImg->CopyInformation(img);
