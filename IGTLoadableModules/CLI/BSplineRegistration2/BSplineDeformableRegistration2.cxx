/*=========================================================================
  ...
  Module:    $HeadURL: http://svn.slicer.org/Slicer3/trunk/Applications/CLI/BSplineDeformableRegistration2.cxx $
  Language:  C++
  Date:      $Date: 2008-11-23 14:01:51 -0500 (Sun, 23 Nov 2008) $
...
#include "BSplineDeformableRegistration2CLP.h"
...
template<class T> int DoIt( int argc, char * argv[], T )
{
  ...
  // Generate and write a field map  -- start
  std::cerr << "Generating field map...." << std::endl;
  
  typedef itk::Point<  float, ImageDimension >  PointType;
  typedef itk::Vector< float, ImageDimension >  VectorType;
  typedef itk::Image< VectorType, ImageDimension >  DeformationFieldType;
  
  typename InputImageType::SpacingType   fixedSpacing    = fixedImageReader->GetOutput()->GetSpacing();
  typename InputImageType::PointType     fixedOrigin     = fixedImageReader->GetOutput()->GetOrigin();
  typename InputImageType::DirectionType fixedDirection  = fixedImageReader->GetOutput()->GetDirection();
  ...
  std::cerr << "Wriging field map...." << std::endl;
  ...
  if( argc >= 6 )
    {
    fieldWriter->SetFileName( "DeformationFieldOut.mhd" );
    try
      {
      fieldWriter->Update();
      }
  ...
  std::cerr << "Wriging field map....done." << std::endl;

  // Generate and write a field map  -- end
