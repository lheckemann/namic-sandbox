...
\title{Estimation of Memory and Computation Time Trade-Off for Deformable
...
\author{Matt Turek$^{1}$, Brad Davis$^{1}$, Serdar Balci$^{2}$, Luis Ibanez$^{1}$, Polina Golland $^{1}$, Stephen Aylward$^{2}$}
...
\begin{abstract}
\noindent
This document describes the trade-offs between memory consumption and
computation time that must be considered when performing deformable
registration using the MattesMutualInformationImageMetric.
