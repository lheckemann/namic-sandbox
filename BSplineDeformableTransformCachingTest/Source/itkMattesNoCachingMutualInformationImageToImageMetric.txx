/*=========================================================================
  ...
  Module:    $RCSfile: itkMattesNoCachingMutualInformationImageToImageMetric.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef _itkMattesNoCachingMutualInformationImageToImageMetric_txx
#define _itkMattesNoCachingMutualInformationImageToImageMetric_txx
...
#include "itkOptMattesNoCachingMutualInformationImageToImageMetric.txx"
#else
...
MattesNoCachingMutualInformationImageToImageMetric<TFixedImage,TMovingImage>
::MattesNoCachingMutualInformationImageToImageMetric()
{
...
// NO LONGER NEEDED    this->PreComputeTransformValues();

    m_Weights.SetSize(m_NumBSplineWeights);
    m_Indices.SetSize(m_NumBSplineWeights);
 ...
 */
template < class TFixedImage, class TMovingImage  >
typename MattesNoCachingMutualInformationImageToImageMetric<TFixedImage,TMovingImage>
::MeasureType
MattesNoCachingMutualInformationImageToImageMetric<TFixedImage,TMovingImage>
  ...
  double& movingImageValue ) const
{
  ...
  if ( !m_TransformIsBSpline )
    ...
    m_BSplineTransform->TransformPoint( m_FixedImageSamples[sampleNumber].FixedImagePointValue,
  ...
  double cubicBSplineDerivativeValue ) const
{
  ...
  if( !m_TransformIsBSpline )
    ...
    {
    //
    // When ComputePDFDerivatives() is called, we have just called TransformPoint() before,
    // and we have stored the values of weights, indices and valid in member variables.
    // Here we just access these member variables.
    ...
    for( unsigned int dim = 0; dim < FixedImageDimension; dim++ )
      ...
      for( unsigned int mu = 0; mu < m_NumBSplineWeights; mu++ )
         ...
         * gradient. */
        double innerProduct = movingImageGradientValue[dim] * m_Weights[mu];
...
// NO LONGER NEEDED  BSplineTransformWeightsType weights( m_NumBSplineWeights );
// NO LONGER NEEDED  BSplineTransformIndexArrayType indices( m_NumBSplineWeights );
// NO LONGER NEEDED:  bool valid;
// NO LONGER NEEDED  MovingImagePointType mappedPoint;
