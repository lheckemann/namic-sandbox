/*=========================================================================
  ...
  Module:    $RCSfile: itkImageToCorrelationMatrix.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef _itkImageToCorrelationMatrix_txx
#define _itkImageToCorrelationMatrix_txx
#include "itkImageToCorrelationMatrix.h"
...
void
ImageToCorrelationMatrix<TInputImage, TOperatorValueType, TOutputValueType>
                            ...
                            int threadId )
{
  ...
  // The output SymmetricSecondRancTensor, which represents the outer product of the gradient,
  ...
  OutputValueType tmp[InputImageDimension];
  OutputValueType compjk[InputImageDimension][InputImageDimension];
  ...
  for (fit=faceList.begin(); fit != faceList.end(); ++fit)
    ...
    while ( ! bit.IsAtEnd() )
      {
      SymmetricSecondRankTensor< OutputValueType,InputImageDimension > a;

      for (i = 0; i < InputImageDimension; ++i)
        {
        tmp[i] = SIP(x_slice[i], bit, op[i]);
        }
      int count = 0;
      for (int j = 0; j < InputImageDimension; ++j)
        {
        for (int k = j; k < InputImageDimension; ++k)
          {
          a[count++] = tmp[j]*tmp[k];
          }
        }
