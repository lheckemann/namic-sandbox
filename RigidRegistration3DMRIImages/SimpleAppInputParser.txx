...
::Execute()
{
  ...
  if( fscanf( paramFile, "%s", currentLine ) != 1 ) 
  {
  itkExceptionMacro( << "Could not find the transform output filename." );
  }

  m_TransformOutputFileName = currentLine;

  std::cout << "Transform output filename: " << m_TransformOutputFileName 
            << std::endl;
  std::cout << std::endl;
