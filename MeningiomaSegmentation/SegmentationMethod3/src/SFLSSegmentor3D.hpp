...
//         labelsCoherentCheck1();
  ...
  ::computeKappa(long ix, long iy, long iz)
  {
    ...
    if (xok)
      {
        dx  = (mp_phi->get(ix+1, iy, iz) - mp_phi->get(ix-1, iy, iz) )/2.0;
        dxx = mp_phi->get(ix+1, iy, iz) - 2.0*(mp_phi->get(ix, iy, iz)) + mp_phi->get(ix-1, iy, iz);
        dx2 = dx*dx;
      }

    if (yok)
      {
        dy  = (mp_phi->get(ix, iy+1, iz) - mp_phi->get(ix, iy-1, iz) )/2.0;
        dyy = mp_phi->get(ix, iy+1, iz) - 2*(mp_phi->get(ix, iy, iz)) + mp_phi->get(ix, iy-1, iz);
        dy2 = dy*dy;
      }

    if (zok)
      {
        dz  = (mp_phi->get(ix, iy, iz+1) - mp_phi->get(ix, iy, iz-1) )/2.0;
        dzz = mp_phi->get(ix, iy, iz+1) - 2.0*(mp_phi->get(ix, iy, iz)) + mp_phi->get(ix, iy, iz-1);
        dz2 = dz*dz;
      }
    ...
    if(xok && yok)
      {
        dxy = 0.25*(mp_phi->get(ix+1, iy+1, iz) + mp_phi->get(ix-1, iy-1, iz) \
                    - mp_phi->get(ix+1, iy-1, iz) - mp_phi->get(ix-1, iy+1, iz));
      }

    if(xok && zok)
      {
        dxz = 0.25*(mp_phi->get(ix+1, iy, iz+1) + mp_phi->get(ix-1, iy, iz-1)\
                    - mp_phi->get(ix+1, iy, iz-1) - mp_phi->get(ix-1, iy, iz+1));
      }

    if(yok && zok)
      {
        dyz = 0.25*(mp_phi->get(ix, iy+1, iz+1) + mp_phi->get(ix, iy-1, iz-1)\
                    - mp_phi->get(ix, iy+1, iz-1) - mp_phi->get(ix, iy-1, iz+1));
      }
