 ...
 */
#ifndef __SVMPR_LOQO_txx
#define __SVMPR_LOQO_txx
...
#define  max_prloqo(A, B)  ((A) > (B) ? (A) : (B))
#define  min_prloqo(A, B)  ((A) < (B) ? (A) : (B))
#define sqr_prloqo(A)          ((A) * (A))
#define  ABS_prloqo(A)    ((A) > 0 ? (A) : (-(A)))
...
{
template<class TVector, class YSample>
void
SVMPrLoqo<TVector,YSample>
...
void
SVMPrLoqo<TVector,YSample>
::cholsb(double a[], int n, double p[], double b[], double x[])
{
  int i, k;
  double sum;
...
void
SVMPrLoqo<TVector,YSample>
::chol_forward(double a[], int n, double p[], double b[], double x[])
{
  int i, k;
  double sum;
...
void
SVMPrLoqo<TVector,YSample>
::chol_backward(double a[], int n, double p[], double b[], double x[])
{
  int i, k;
  double sum;
...
void
SVMPrLoqo<TVector,YSample>
::solve_reduced(int n, int m, double h_x[], double h_y[], 
...
void
SVMPrLoqo<TVector,YSample>
::matrix_vector(int n, double m[], double x[], double y[])
{
  int i, j;
...
int
SVMPrLoqo<TVector,YSample>
::pr_loqo(int n, int m, double c[], double h_x[], double a[], double b[],
      ...
      double margin, double bound, int restart) 
{
  ...
  if (restart == 1) {
    ...
    for (i=0; i<n; i++) {  /* compute g, t for primal feasibility */
      g[i] = max_prloqo(ABS_prloqo(x[i] - l[i]), bound);
      t[i] = max_prloqo(ABS_prloqo(u[i] - x[i]), bound); 
    }
  ...
  h_y[i*m + j] = (i==j) ? 1 : 0;
    ...
    for (i=0; i<n; i++) {
      g[i] = max_prloqo(ABS_prloqo(x[i] - l[i]), bound);
      z[i] = max_prloqo(ABS_prloqo(x[i]), bound);
      t[i] = max_prloqo(ABS_prloqo(u[i] - x[i]), bound); 
      s[i] = max_prloqo(ABS_prloqo(x[i]), bound); 
    }
  }
  ...
  sigma[i] -= a[n*j + i] * y[j];
    ...
    for (i=0; i<n; i++) {
      x_h_x += h_dot_x[i] * x[i];
      primal_inf += sqr_prloqo(tau[i]);
      primal_inf += sqr_prloqo(nu[i]);
      dual_inf += sqr_prloqo(sigma[i]);
    }
    for (i=0; i<m; i++) 
      primal_inf += sqr_prloqo(rho[i]);
    ...
    sigfig = log10(ABS_prloqo(primal_obj) + 1) -
             log10(ABS_prloqo(primal_obj - dual_obj));
    sigfig = max_prloqo(sigfig, 0);
    ...
    if (ABS_prloqo(primal_obj) > 10e100) status = PRIMAL_UNBOUNDED;
    if (ABS_prloqo(dual_obj) > 10e100) status = DUAL_UNBOUNDED;
  ...
  delta_t[i] = t[i] * (gamma_s[i] - delta_s[i]) / s[i];
      ...
      alfa = -1;
      for (i=0; i<n; i++) {
  alfa = min_prloqo(alfa, delta_g[i]/g[i]);
  alfa = min_prloqo(alfa, delta_t[i]/t[i]);
  alfa = min_prloqo(alfa, delta_s[i]/s[i]);
  alfa = min_prloqo(alfa, delta_z[i]/z[i]);
      }
      alfa = (margin - 1) / alfa;
  ...
  mu += z[i] * g[i] + s[i] * t[i];
      mu = mu / (2*n);
      mu = mu * sqr_prloqo((alfa - 1) / (alfa + 10));
