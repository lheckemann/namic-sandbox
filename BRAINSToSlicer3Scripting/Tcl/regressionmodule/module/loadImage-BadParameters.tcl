...
# \brief    This module tests the bad parameters for b2 load image so that it is not done in all other load image files.
...
########################################
    ...
    set ModuleDescription "Test the b2 load image command for bad parameters." ;
...
# First Test for invalid arguments
    ...
    set ErrorReturnOK [b2 load image $pathToRegressionDir/SGI/MR/4x-B1/TEST/15_002/19771.002.096 junk= ] ;
    ReportTestStatus $LogFile  [ expr {$ErrorReturnOK == -1 } ] $ModuleName $SubTestDes ;

    set SubTestDes "optional argument test" ;
    set ErrorReturnOK [b2 load image $pathToRegressionDir/SGI/MR/4x-B1/TEST/15_002/19771.002.096 junk= test] ;
    ReportTestStatus $LogFile  [ expr {$ErrorReturnOK == -1 } ] $ModuleName $SubTestDes ;
