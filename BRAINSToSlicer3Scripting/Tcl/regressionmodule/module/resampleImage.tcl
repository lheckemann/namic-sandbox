...
# \brief    This module tests the "b2 resample image"
...
# Test Performed
# -----------------------------------------------------------------------
# Test the b2 threshold image command
#
# To Do
#------------------------------------------------------------------------
# None
...
proc resampleImage {pathToRegressionDir dateString} {
    ...
    set ModuleDescription "Test the b2 resample image command"
...
########################################
    ...
    set ImageId [b2 load image $pathToRegressionDir/SGI/MR/4x-B1/TEST/15_002/19771.002.042]
    if { [ ReportTestStatus $LogFile  [ expr {$ImageId != -1 } ] $ModuleName $SubTestDes] == 0} {
        return $MODULE_FAILURE
    }

    set SubTestDes "Resample Image - Load Transform test"
    set TransformId [b2 load transform $pathToRegressionDir/SGI/MR/4x-B1/TEST/10_ACPC/resample_T1.xfrm]
    if { [ ReportTestStatus $LogFile  [ expr {$ImageId != -1 } ] $ModuleName $SubTestDes] == 0} {
        return $MODULE_FAILURE
    }
    ...
    set errorTest [b2 resample image]
    ReportTestStatus $LogFile  [ expr {$errorTest == -1 } ] $ModuleName $SubTestDes

    set SubTestDes "argument number test"
    set errorTest [b2 resample image $ImageId 100 junk= ]
    ReportTestStatus $LogFile  [ expr {$errorTest == -1 } ] $ModuleName $SubTestDes

    set SubTestDes "optional argument test"
    set errorTest [b2 resample image $ImageId junk= test]
    ReportTestStatus $LogFile  [ expr {$errorTest == -1 } ] $ModuleName $SubTestDes

    set SubTestDes "invalid image test"
    set errorTest [b2 resample image 65536]
    ReportTestStatus $LogFile  [ expr {$errorTest == -1 } ] $ModuleName $SubTestDes

    b2 set transform $TransformId image $ImageId

    ############################ BRAINS2 Resample Image ########################################
    set SubTestDes "B2 RESAMPLE IMAGE test"
    set TestImageId [b2 resample image $ImageId plane= coronal]
    ReportTestStatus $LogFile  [ expr {$TestImageId != -1 } ] $ModuleName $SubTestDes
    if { $TestImageId != -1 } {
    set SubTestDes "B2 RESAMPLE IMAGE test - Mask Generation"
    set TestMaskId [b2 threshold image $TestImageId 50]
    ...
    set TestImageId [b2 resample image $ImageId plane= coronal interp= nearest-neighbor]
    ReportTestStatus $LogFile  [ expr {$TestImageId != -1 } ] $ModuleName $SubTestDes
    if { $TestImageId != -1 } {
    set result [b2 measure image mask $TestMaskId $TestImageId]
    ...
    set TestImageId [b2 resample image $ImageId min= 126 max= 128 plane= axial storage-type= float-single]
    ReportTestStatus $LogFile  [ expr {$TestImageId != -1 } ] $ModuleName $SubTestDes
    if { $TestImageId != -1 } {
    set SubTestDes "B2 RESAMPLE IMAGE test - Mask Generation"
    set TestMaskId [b2 threshold image $TestImageId 50]
    ...
    ReportTestStatus $LogFile  [ expr { [ b2 destroy transform $TransformId ] != -1 } ] $ModuleName "Destroying transform $TransformId"
