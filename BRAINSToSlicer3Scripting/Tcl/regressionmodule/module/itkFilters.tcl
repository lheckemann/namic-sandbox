...
ReportTestStatus $LogFile  [ expr { [ b2 destroy every roi ] != -1 } ] $ModuleName "Destroying every roi"
ReportTestStatus $LogFile  [ expr { [ b2 destroy every image ] != -1 } ] $ModuleName "Destroying every image"
    ...
    set TestImageID [b2 load image $pathToRegressionDir/SGI/MR/4x-B1/TEST/10_ACPC/ANON013_T1_boxed.hdr]
    ...
    set SubTestDes "Load requisite objects for filter testing - whole box mask"
      set brainMask [b2 load mask $pathToRegressionDir/SGI/MR/4x-B1/TEST/10_ACPC/ANON013_boxed.mask ]
...
set output [ b2 measure image mask $brainMask $TestImageID]
...
set filtImg4F [b2 itkRecursiveGaussian $TestImageID 4.0 data-type= float-single]
set output4F [b2 measure image mask $brainMask $filtImg4F]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImg4F ] != -1 } ] $ModuleName "Destroying image $filtImg4F"
#######################################################################################################################
#######################################################################################################################

set filtImg4B [b2 itkRecursiveGaussian $TestImageID 4.0 data-type= Unsigned-8bit]
set output4B [b2 measure image mask $brainMask $filtImg4B]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImg4B ] != -1 } ] $ModuleName "Destroying image $filtImg4B"
#######################################################################################################################
#######################################################################################################################

set filtImgDG [b2 itkDiscreteGaussian $TestImageID 4.0 12 data-type= float-single]
set outputDG [b2 measure image mask $brainMask $filtImgDG]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgDG ] != -1 } ] $ModuleName "Destroying image $filtImgDG"
#######################################################################################################################
#######################################################################################################################

set filtImgSig [b2 itkSigmoid $TestImageID -8.33333333333 65.0 data-type= float-single]
set outputSig [b2 measure image mask $brainMask $filtImgSig]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgSig ] != -1 } ] $ModuleName "Destroying image $filtImgSig"
#######################################################################################################################
#######################################################################################################################

set filtImgMed [b2 itkMedian $TestImageID 1 data-type= float-single]
set outputMed [b2 measure image mask $brainMask $filtImgMed]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgMed ] != -1 } ] $ModuleName "Destroying image $filtImgMed"
#######################################################################################################################
#######################################################################################################################

set filtImgGAD [b2 itkGradientAnisotropicDiffusion $TestImageID iterations= 4 conductance= 0.7 data-type= float-single]
set outputGAD [b2 measure image mask $brainMask $filtImgGAD]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgGAD ] != -1 } ] $ModuleName "Destroying image $filtImgGAD"
#######################################################################################################################
#######################################################################################################################

set filtImgBilat [b2 itkBilateral $TestImageID 1.0 12.0 data-type= float-single]
set outputBilat [b2 measure image mask $brainMask $filtImgBilat]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgBilat ] != -1 } ] $ModuleName "Destroying image $filtImgBilat"
#######################################################################################################################
#######################################################################################################################

set filtImgGM [b2 itkGradientMagnitude $TestImageID data-type= float-single]
set outputGM [b2 measure image mask $brainMask $filtImgGM]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filtImgGM ] != -1 } ] $ModuleName "Destroying image $filtImgGM"
#######################################################################################################################
#######################################################################################################################

set outerMask [b2 threshold image $TestImageID 80]
set innerMask [b2 not mask $outerMask]
set outerImage [b2 sum masks [list $outerMask]]
set innerDistImg [b2 itkDanielssonDistanceMap $outerImage data-type= float-single]

set outputInnerDist [b2 measure image mask $innerMask $innerDistImg]
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $innerDistImg ] != -1 } ] $ModuleName "Destroying image $innerDistImg"
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $outerImage ] != -1 } ] $ModuleName "Destroying image $outerImage"
ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $outerMask ] != -1 } ] $ModuleName "Destroying mask $outerMask"
ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $innerMask ] != -1 } ] $ModuleName "Destroying mask $innerMask"
#######################################################################################################################
#######################################################################################################################
#######################################################################################################################
#######################################################################################################################
