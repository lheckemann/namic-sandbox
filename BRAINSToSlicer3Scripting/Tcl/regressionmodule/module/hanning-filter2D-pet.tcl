...
proc hanning-filter2D-pet { pathToRegressionDir dateString } {
        ...
        set ModuleDescription "Testing of b2 hanning-filter2D-pet filter"
...
#set pathToRegressionDir /scratch/purify/src/regressiontest
    ...
    set TestImageID [b2 load image $pathToRegressionDir/SGI/phantom/AnalyzeImages/impulse.hdr]
      if { ! [ ReportTestStatus $LogFile  [ expr {$TestImageID != -1 } ] $ModuleName $SubTestDes ]} {
          puts "Error reading image.\nFailed to load Image."
      }

set all_mask [ b2 threshold image $TestImageID 0.00 absolute-value= True ]


## historical mode 4mm filter
set filter_4mm_h [ b2 hanning-filter2D-pet $TestImageID filter-size= 4.1 historical-mode= 1 ]
set output [ b2 measure image mask $all_mask $filter_4mm_h ]
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filter_4mm_h ] != -1 } ] $ModuleName "Destroying image $filter_4mm_h"
...
set filter_0mm_h [ b2 hanning-filter2D-pet $TestImageID filter-size= 0 historical-mode= 1 ]
set output [ b2 measure image mask $all_mask $filter_0mm_h ]
set known_output {
...
set subtract_0mm_h  [ b2 subtract images [ list $filter_0mm_h $TestImageID ] ]
set output [ b2 measure image mask $all_mask $subtract_0mm_h ]
set known_output {
...
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filter_0mm_h ] != -1 } ] $ModuleName "Destroying image $filter_0mm_h"
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $subtract_0mm_h ] != -1 } ] $ModuleName "Destroying image $subtract_0mm_h"

## historical mode 4mm filter
set filter_4mm [ b2 hanning-filter2D-pet $TestImageID filter-size= 4.1 historical-mode= 0 ]
set output [ b2 measure image mask $all_mask $filter_4mm ]
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filter_4mm ] != -1 } ] $ModuleName "Destroying image $filter_4mm"
...
## historical mode 0mm filter
set filter_0mm [ b2 hanning-filter2D-pet $TestImageID filter-size= 0 historical-mode= 0 ]
set output [ b2 measure image mask $all_mask $filter_0mm ]
set known_output {
{Mean 0.003051757812500} {Mean-Absolute-Deviation 0.006103329360485} {Variance 0.305175781250000} {Standard-Deviation 0.552427172801990} {Skewness 181.002763842861270} {Kurtosis 32761.000183102190931} {Minimum 0.000000000000000} {Maximum 100.000000000000000} {Min-Loc-X 0} {Min-Loc-Y 0} {Min-Loc-Z 0} {Max-Loc-X 15} {Max-Loc-Y 15} {Max-Loc-Z 15}
 }
CoreMeasuresEpsilonTest "Hanning 0mm historical-mode=0" 0.0001 $known_output $output $LogFile $ModuleName
set subtract_0mm  [ b2 subtract images [ list $filter_0mm $TestImageID ] ]
set output [ b2 measure image mask $all_mask $subtract_0mm ]
set known_output {
{Mean 0.000000000000000} {Mean-Absolute-Deviation 0.000000000000000} {Variance 0.000000000000000} {Standard-Deviation 0.000000000000000} {Skewness 0.000000000000000} {Kurtosis 0.000000000000000} {Minimum 0.000000000000000} {Maximum 0.000000000000000} {Min-Loc-X 0} {Min-Loc-Y 0} {Min-Loc-Z 0} {Max-Loc-X 0} {Max-Loc-Y 0} {Max-Loc-Z 0}
}
CoreMeasuresEpsilonTest "subtraction 0mm historical-mode=0" 0.0001 $known_output $output $LogFile $ModuleName
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $filter_0mm ] != -1 } ] $ModuleName "Destroying image $filter_0mm"
ReportTestStatus $LogFile  [ expr { [ b2 destroy image $subtract_0mm ] != -1 } ] $ModuleName "Destroying image $subtract_0mm"


    ReportTestStatus $LogFile  [ expr { [ b2 destroy image $TestImageID ] != -1 } ] $ModuleName "Destroying image $TestImageID"
    ReportTestStatus $LogFile  [ expr { [ b2 destroy mask $all_mask ] != -1 } ] $ModuleName "Destroying mask $all_mask"
