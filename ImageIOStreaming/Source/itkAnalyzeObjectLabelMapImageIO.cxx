...
AnalyzeObjectLabelMapImageIO::AnalyzeObjectLabelMapImageIO():
  m_AnalyzeObjectLabelMapImage(0)
{
...
AnalyzeObjectLabelMapImageIO::~AnalyzeObjectLabelMapImageIO()
{
  nifti_image_free(this->m_AnalyzeObjectLabelMapImage);
...
void AnalyzeObjectLabelMapImageIO::Read(void* buffer)
{
  ...
  this->m_AnalyzeObjectLabelMapImage = nifti_image_read(m_FileName.c_str(),false);

  if(nifti_read_subregion_image(this->m_AnalyzeObjectLabelMapImage,
                                _origin,
                                _size,
                                &data) == -1 || this->m_AnalyzeObjectLabelMapImage == NULL)
    {
  ...
  if(numComponents == 1 || this->GetPixelType() == COMPLEX)
    {
    const size_t NumBytes=numElts * this->m_AnalyzeObjectLabelMapImage->nbyper;
    memcpy(buffer, data, NumBytes);
    }
  else
    {
    unsigned nbyper = this->m_AnalyzeObjectLabelMapImage->nbyper;
    //TODO:  Need to coerse these dims based on the collapsed areas.
    int *dim = this->m_AnalyzeObjectLabelMapImage->dim;
...
void AnalyzeObjectLabelMapImageIO::ReadImageInformation()
{
  ...
  if(this->m_AnalyzeObjectLabelMapImage == 0)
    {
    ExceptionObject exception(__FILE__, __LINE__);
  ...
  this->SetNumberOfDimensions(this->m_AnalyzeObjectLabelMapImage->ndim);
  switch( this->m_AnalyzeObjectLabelMapImage->datatype )
    {
  ...
  switch(XYZT_TO_SPACE(this->m_AnalyzeObjectLabelMapImage->xyz_units))
    {
  ...
  switch(XYZT_TO_TIME(this->m_AnalyzeObjectLabelMapImage->xyz_units))
    {
  ...
  // vector images? 
  if((this->m_AnalyzeObjectLabelMapImage->dim[0] == 5 && this->m_AnalyzeObjectLabelMapImage->dim[5] > 1))
    {
    dims = 4;                   // as far as ITK is concerned, the dimension
    // should now be 4
    //
    // each pixel is a vector
    this->SetNumberOfComponents(this->m_AnalyzeObjectLabelMapImage->nu);
    }
  switch(dims)
    {
    case 7:
      this->SetDimensions(6,this->m_AnalyzeObjectLabelMapImage->nw);
      this->SetSpacing(6,this->m_AnalyzeObjectLabelMapImage->dw);//NOTE: Scaling is not defined in this dimension
    case 6:
      this->SetDimensions(5,this->m_AnalyzeObjectLabelMapImage->nv);
      this->SetSpacing(5,this->m_AnalyzeObjectLabelMapImage->dv);//NOTE: Scaling is not defined in this dimension
    case 5:
      this->SetDimensions(4,this->m_AnalyzeObjectLabelMapImage->nu);
      this->SetSpacing(4,this->m_AnalyzeObjectLabelMapImage->du);//NOTE: Scaling is not defined in this dimension
    case 4:
      this->SetDimensions(3,this->m_AnalyzeObjectLabelMapImage->nt);
      this->SetSpacing(3,this->m_AnalyzeObjectLabelMapImage->dt*timingscale);
    case 3:
      this->SetDimensions(2,this->m_AnalyzeObjectLabelMapImage->nz);
      this->SetSpacing(2,this->m_AnalyzeObjectLabelMapImage->dz*spacingscale);
    case 2:
      this->SetDimensions(1,this->m_AnalyzeObjectLabelMapImage->ny);
      this->SetSpacing(1,this->m_AnalyzeObjectLabelMapImage->dy*spacingscale);
    case 1:
      this->SetDimensions(0,this->m_AnalyzeObjectLabelMapImage->nx);
      this->SetSpacing(0,this->m_AnalyzeObjectLabelMapImage->dx*spacingscale);
    }
  this->ComputeStrides();
  ...
  if(this->m_AnalyzeObjectLabelMapImage->qform_code == 0 && this->m_AnalyzeObjectLabelMapImage->sform_code == 0)
    {
    SpatialOrientationAdapter::OrientationType orient;
    switch(this->m_AnalyzeObjectLabelMapImage->analyze75_orient)
      {
    ...
    //
    mat44 theMat;
    if(this->m_AnalyzeObjectLabelMapImage->qform_code > 0)
      {
      theMat = this->m_AnalyzeObjectLabelMapImage->qto_xyz;
      }
    //    else if(this->m_AnalyzeObjectLabelMapImage->sform_code > 0)
    else
      {
      theMat = this->m_AnalyzeObjectLabelMapImage->sto_xyz;
      }
    int ii, jj, kk;
    ...
    //
    if((m_RescaleSlope = this->m_AnalyzeObjectLabelMapImage->scl_slope) == 0)
      {
      m_RescaleSlope = 1;
      }
    m_RescaleIntercept = this->m_AnalyzeObjectLabelMapImage->scl_inter;
    //
    // set origin
...
//   std::cerr << "read: dirz " << dirz[0] << " " << dirz[1] << " " << dirz[2] << std::endl;
  ...
  std::string description(this->m_AnalyzeObjectLabelMapImage->descrip);
  EncapsulateMetaData<std::string>(this->GetMetaDataDictionary(),
                                    ITK_FileNotes,description);

  // We don't need the image anymore
  nifti_image_free(this->m_AnalyzeObjectLabelMapImage);
  this->m_AnalyzeObjectLabelMapImage = 0;
}
...
::WriteImageInformation(void) //For Nifti this does not write a file, it only fills in the appropriate header information.
{
  ...
  if(this->m_AnalyzeObjectLabelMapImage == 0)
    {
    this->m_AnalyzeObjectLabelMapImage = nifti_simple_init_nim();
    }
  //
  // set the filename
  std::string FName(this->GetFileName());
  this->m_AnalyzeObjectLabelMapImage->fname = (char *)malloc(FName.size()+1);
  strcpy(this->m_AnalyzeObjectLabelMapImage->fname,FName.c_str());
  //
  // set the file type
  std::string::size_type ext = FName.rfind('.');
  if(ext == std::string::npos)
  ...
  if(Ext == ".nii" || Ext == ".nii.gz")
    {
    this->m_AnalyzeObjectLabelMapImage->nifti_type = 1;
    this->m_AnalyzeObjectLabelMapImage->iname = (char *)malloc(FName.size()+1);
    strcpy(this->m_AnalyzeObjectLabelMapImage->fname,FName.c_str());
    strcpy(this->m_AnalyzeObjectLabelMapImage->iname,FName.c_str());
    }
  else if(Ext == ".hdr" || Ext == ".img")
    {
    this->m_AnalyzeObjectLabelMapImage->nifti_type = 2;
    if(Ext == ".hdr")
      {
      strcpy(this->m_AnalyzeObjectLabelMapImage->fname,FName.c_str());
      }
  ...
  unsigned short dims =
    this->m_AnalyzeObjectLabelMapImage->ndim =
    this->m_AnalyzeObjectLabelMapImage->dim[0] =
...
//                   dim[3] for 3-D volumes, etc.
  this->m_AnalyzeObjectLabelMapImage->nvox = 1;
  ...
  //      to seconds.
  this->m_AnalyzeObjectLabelMapImage->xyz_units=NIFTI_UNITS_MM | NIFTI_UNITS_SEC;
  if(this->GetNumberOfComponents() > 1 && !(this->GetPixelType() == COMPLEX &&
                                            this->GetNumberOfComponents() == 2))
    {
    this->m_AnalyzeObjectLabelMapImage->intent_code = NIFTI_INTENT_VECTOR;
    //
    // we're bumping dim to 5, so make sure dim 4 is 1 if we're coming from dim < 4
    if(dims < 4)
      {
      this->m_AnalyzeObjectLabelMapImage->nt =
      this->m_AnalyzeObjectLabelMapImage->dim[4] = 1;
      }
    if(dims < 3)
      {
      this->m_AnalyzeObjectLabelMapImage->nz =
      this->m_AnalyzeObjectLabelMapImage->dim[3] = 1;
      }
    dims = this->m_AnalyzeObjectLabelMapImage->ndim = this->m_AnalyzeObjectLabelMapImage->dim[0] = 5; // has to be 5 
    this->m_AnalyzeObjectLabelMapImage->nu =
    this->m_AnalyzeObjectLabelMapImage->dim[5] = this->GetNumberOfComponents();
    }
  switch(origdims)
    {
    case 7:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[7] =
        this->m_AnalyzeObjectLabelMapImage->nw = this->GetDimensions(6);
      this->m_AnalyzeObjectLabelMapImage->pixdim[7] = this->m_AnalyzeObjectLabelMapImage->dw =
        this->GetSpacing(6);
    case 6:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[6] =
        this->m_AnalyzeObjectLabelMapImage->nv = this->GetDimensions(5);
      this->m_AnalyzeObjectLabelMapImage->pixdim[6] = this->m_AnalyzeObjectLabelMapImage->dv =
        this->GetSpacing(5);
    case 5:
      this->m_AnalyzeObjectLabelMapImage->dim[5] =
        this->m_AnalyzeObjectLabelMapImage->nu = this->GetDimensions(4);
      this->m_AnalyzeObjectLabelMapImage->pixdim[5] =
        this->m_AnalyzeObjectLabelMapImage->du = this->GetSpacing(4);
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[5];
    case 4:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[4] =
        this->m_AnalyzeObjectLabelMapImage->nt = this->GetDimensions(3);
      this->m_AnalyzeObjectLabelMapImage->pixdim[4] =
        this->m_AnalyzeObjectLabelMapImage->dt = this->GetSpacing(3);
    case 3:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[3] =
        this->m_AnalyzeObjectLabelMapImage->nz = this->GetDimensions(2);
      this->m_AnalyzeObjectLabelMapImage->pixdim[3] =
        this->m_AnalyzeObjectLabelMapImage->dz = this->GetSpacing(2);
    case 2:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[2] =
        this->m_AnalyzeObjectLabelMapImage->ny = this->GetDimensions(1);
      this->m_AnalyzeObjectLabelMapImage->pixdim[2] =
        this->m_AnalyzeObjectLabelMapImage->dy = this->GetSpacing(1);
    case 1:
      this->m_AnalyzeObjectLabelMapImage->nvox *= this->m_AnalyzeObjectLabelMapImage->dim[1] =
        this->m_AnalyzeObjectLabelMapImage->nx = this->GetDimensions(0);
      this->m_AnalyzeObjectLabelMapImage->pixdim[1] =
        this->m_AnalyzeObjectLabelMapImage->dx = this->GetSpacing(0);
    }
  ...
  switch(this->GetComponentType())
    {
    case UCHAR:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_UINT8;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 1;
      break;
    case CHAR:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_INT8;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 1;
      break;
    case USHORT:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_UINT16;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 2;
      break;
    case SHORT:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_INT16;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 2;
      break;
    case ULONG:
    case UINT:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_UINT32;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 4;
      break;
    case LONG:
    case INT:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_INT32;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 4;
      break;
    case FLOAT:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_FLOAT32;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 4;
      break;
    case DOUBLE:
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_FLOAT64;
      this->m_AnalyzeObjectLabelMapImage->nbyper = 8;
      break;
  ...
  switch(this->GetPixelType())
    ...
    case SCALAR:
      break;
    case RGB:
      this->m_AnalyzeObjectLabelMapImage->nbyper *= 3;
      this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_RGB24;
      break;
    case COMPLEX:
      this->m_AnalyzeObjectLabelMapImage->nbyper *= 2;
      switch(this->GetComponentType())
        {
        case FLOAT:
          this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_COMPLEX64;
          break;
        case DOUBLE:
          this->m_AnalyzeObjectLabelMapImage->datatype = NIFTI_TYPE_COMPLEX128;
          break;
        default:
          {
          ExceptionObject exception(__FILE__, __LINE__);
      ...
      break;
    default:
      this->m_AnalyzeObjectLabelMapImage->nbyper *= this->GetNumberOfComponents();
      break;
    }
  //     -----------------------------------------------------
  ...
  //     -----------------------------------------------------
  this->m_AnalyzeObjectLabelMapImage->scl_slope = 1.0;
  this->m_AnalyzeObjectLabelMapImage->scl_inter = 0.0;

  //
  // use NIFTI method 2
  this->m_AnalyzeObjectLabelMapImage->sform_code = NIFTI_XFORM_SCANNER_ANAT;
  this->m_AnalyzeObjectLabelMapImage->qform_code = NIFTI_XFORM_ALIGNED_ANAT;
  ...
  nifti_mat44_to_quatern(matrix,
                         &(this->m_AnalyzeObjectLabelMapImage->quatern_b),
                         &(this->m_AnalyzeObjectLabelMapImage->quatern_c),
                         &(this->m_AnalyzeObjectLabelMapImage->quatern_d),
                         &(this->m_AnalyzeObjectLabelMapImage->qoffset_x),
                         &(this->m_AnalyzeObjectLabelMapImage->qoffset_y),
                         &(this->m_AnalyzeObjectLabelMapImage->qoffset_z),
                         0,
                         0,
                         0,
                         &(this->m_AnalyzeObjectLabelMapImage->qfac));
  // copy q matrix to s matrix
  this->m_AnalyzeObjectLabelMapImage->qto_xyz =  matrix;
  this->m_AnalyzeObjectLabelMapImage->sto_xyz =  matrix;
  for(unsigned i = 0; i < 3; i++)
    {
    for(unsigned j = 0; j < 3; j++)
      {
      this->m_AnalyzeObjectLabelMapImage->sto_xyz.m[i][j] = this->GetSpacing(j) *
        this->m_AnalyzeObjectLabelMapImage->sto_xyz.m[i][j];
      this->m_AnalyzeObjectLabelMapImage->sto_ijk.m[i][j] = 
        this->m_AnalyzeObjectLabelMapImage->sto_xyz.m[i][j] / this->GetSpacing(j);
      }
    }
  this->m_AnalyzeObjectLabelMapImage->sto_ijk =  
    nifti_mat44_inverse(this->m_AnalyzeObjectLabelMapImage->sto_xyz);
  this->m_AnalyzeObjectLabelMapImage->qto_ijk =  
    nifti_mat44_inverse(this->m_AnalyzeObjectLabelMapImage->qto_xyz);
  
  this->m_AnalyzeObjectLabelMapImage->pixdim[0] = this->m_AnalyzeObjectLabelMapImage->qfac;
  this->m_AnalyzeObjectLabelMapImage->qform_code = NIFTI_XFORM_SCANNER_ANAT;
  //  this->m_AnalyzeObjectLabelMapImage->sform_code = 0;
  return;
}
...
::Write( const void* buffer)
{
  ...
  if(numComponents == 1)
    {
    this->m_AnalyzeObjectLabelMapImage->data=const_cast<void *>(buffer);//Need a const cast here so that we don't have to copy the memory for writing.
    nifti_image_write(this->m_AnalyzeObjectLabelMapImage);
    this->m_AnalyzeObjectLabelMapImage->data = 0; // if left pointing to data buffer
    ...
    this->m_AnalyzeObjectLabelMapImage->data=static_cast<void *>(tobuffer);
    nifti_image_write(this->m_AnalyzeObjectLabelMapImage);
    this->m_AnalyzeObjectLabelMapImage->data = 0; // if left pointing to data buffer
    delete [] tobuffer;
    }
