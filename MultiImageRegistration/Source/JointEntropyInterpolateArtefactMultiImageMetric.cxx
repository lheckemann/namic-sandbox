...
=========================================================================*/
#ifndef _JointEntropyInterpolateArtefactMultiImageMetric_cxx
#define _JointEntropyInterpolateArtefactMultiImageMetric_cxx
...
JointEntropyInterpolateArtefactMultiImageMetric < TFixedImage >::
...
::Initialize(void) throw ( ExceptionObject )
{
  ...
  interpolationVariance.resize(this->m_NumberOfSpatialSamples);
  for(int i=0; i<this->m_NumberOfSpatialSamples;i++)
  {
    interpolationVariance[i].resize(this->m_NumberOfImages);
  }
  
  for(int i=0; i<this->m_NumberOfImages; i++)
  {
    if( !this->m_MutualInformationInterpolatorArray[i] )
    {
      itkExceptionMacro(<<"Mutual Information Interpolator is not present");
    }
 ...
 */
template < class TFixedImage >
typename JointEntropyInterpolateArtefactMultiImageMetric < TFixedImage >::MeasureType
JointEntropyInterpolateArtefactMultiImageMetric <TFixedImage >::
  ...
  for(int i=0; i< this->m_NumberOfSpatialSamples; i++ )
  {
    for(int j=0; j<this->m_NumberOfImages; j++)
    {
      ...
      if(this->m_InterpolatorArray[j]->IsInsideBuffer (this->m_Sample[i].mappedPointsArray[j]) )
      {
        ...
        // Get the variance of the points value
        interpolationVariance[i][j] =
            this->m_MutualInformationInterpolatorArray[j]->GetSumOfSquaresOfCoefficients(this->m_Sample[i].mappedPointsArray[j]);
      }
    }
  }
...
::BeforeGetThreadedValueAndDerivative(const ParametersType & parameters) const
{
   ...
   // Populate the interpolation weights array
   for(int i=0; i<this->m_NumberOfSpatialSamples; i++)
   {
     for(int j=0; j<this->m_NumberOfImages; j++)
     {
...
template < class TFixedImage >
void JointEntropyInterpolateArtefactMultiImageMetric < TFixedImage >
