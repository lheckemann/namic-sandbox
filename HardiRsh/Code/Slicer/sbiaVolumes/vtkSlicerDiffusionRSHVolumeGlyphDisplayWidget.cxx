...
#include "vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget.h"
...
//---------------------------------------------------------------------------
vtkStandardNewMacro (vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget );
vtkCxxRevisionMacro ( vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget, "$Revision: 1.0 $");


//---------------------------------------------------------------------------
vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget ( )
  {

  this->DiffusionRSHVolumeNode = NULL;
...
vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::~vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget ( )
  ...
  vtkSetAndObserveMRMLNodeMacro(this->DiffusionRSHVolumeNode, NULL);
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::PrintSelf ( ostream& os, vtkIndent indent )
  {
  this->vtkObject::PrintSelf ( os, indent );

  os << indent << "vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget: " << this->GetClassName ( ) << "\n";
  // print widgets?
  }

//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::SetDiffusionRSHVolumeNode ( vtkMRMLDiffusionRSHVolumeNode *node )
  { 
  if (node == this->DiffusionRSHVolumeNode)
    {
    return;
    }
  vtkSetAndObserveMRMLNodeMacro(this->DiffusionRSHVolumeNode, node);

  if (this->DiffusionRSHVolumeNode )
    {
    vtkMRMLDiffusionRSHVolumeDisplayNode* displayNode = vtkMRMLDiffusionRSHVolumeDisplayNode::SafeDownCast( this->DiffusionRSHVolumeNode->GetDisplayNode() );
    if ( displayNode )
      {
      std::vector< vtkMRMLGlyphableVolumeSliceDisplayNode* > displayNodes = displayNode->GetSliceGlyphDisplayNodes(this->DiffusionRSHVolumeNode);
      this->GlyphDisplayNodes.clear();
      for( size_t i = 0 ; i<displayNodes.size(); i++ )
      {
        vtkMRMLDiffusionRSHVolumeSliceDisplayNode* dtVolumeSliceDisplayNode = vtkMRMLDiffusionRSHVolumeSliceDisplayNode::SafeDownCast( displayNodes[i] );
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::ProcessWidgetEvents ( vtkObject *caller,
                                                                            unsigned long event, void *callData )
  {

  if (this->MRMLScene && this->DiffusionRSHVolumeNode != NULL  &&
  ...
  if (vtkKWMenu::SafeDownCast(caller) && vtkKWMenu::SafeDownCast(caller) == this->GeometryColorMenu->GetWidget()->GetMenu() &&
    ...
    if( loc != std::string::npos ) 
      ...
      for (unsigned int i=0; i<this->GlyphDisplayNodes.size(); i++)
        {
        vtkMRMLDiffusionRSHVolumeSliceDisplayNode* dnode = this->GlyphDisplayNodes[i];
        if (dnode != NULL)
          {
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::UpdateMRML()
  {
  if (this->DiffusionRSHVolumeNode == NULL)
    {
    return;
    }
  ...
  vtkMRMLDiffusionRSHDisplayPropertiesNode* propNode = this->GetCurrentDiffusionRSHDisplayPropertyNode();
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::ProcessMRMLEvents ( vtkObject *caller,
                                                                          unsigned long event, void *callData )
  {
  ...
  vtkMRMLDiffusionRSHVolumeNode *node = vtkMRMLDiffusionRSHVolumeNode::SafeDownCast(caller);
  ...
  if (node != NULL && event == vtkCommand::ModifiedEvent)
    {
    this->SetDiffusionRSHVolumeNode(node);
    }
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::AddMRMLObservers ( )
  {
  if (this->DiffusionRSHVolumeNode != NULL)
    {
    this->DiffusionRSHVolumeNode->AddObserver(vtkCommand::ModifiedEvent,
      (vtkCommand *)this->MRMLCallbackCommand );      
    }
  }

//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::RemoveMRMLObservers ( )
  {
  if (this->DiffusionRSHVolumeNode != NULL)
    {
    this->DiffusionRSHVolumeNode->RemoveObservers(vtkCommand::ModifiedEvent,
      (vtkCommand *)this->MRMLCallbackCommand );      
    }
  }

//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::UpdateWidget()
  {
  if (this->UpdatingMRML || this->UpdatingWidget)
    {
    return;
  ...
  vtkMRMLDiffusionRSHDisplayPropertiesNode *dpnode = this->GetCurrentDiffusionRSHDisplayPropertyNode();
  this->GlyphDisplayWidget->SetDiffusionRSHDisplayPropertiesNode(dpnode);
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::RemoveWidgetObservers ( ) {
...
//---------------------------------------------------------------------------
void vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::CreateWidget ( )
  {
  ...
  */
  ...
  this->GlyphDisplayWidget = vtkSlicerDiffusionRSHGlyphDisplayWidget::New();
...
//---------------------------------------------------------------------------
vtkMRMLDiffusionRSHDisplayPropertiesNode* vtkSlicerDiffusionRSHVolumeGlyphDisplayWidget::GetCurrentDiffusionRSHDisplayPropertyNode()
  {
  vtkMRMLDiffusionRSHDisplayPropertiesNode *dpnode = NULL;
  if (this->GlyphDisplayNodes.size() > 0)
    {
    vtkMRMLDiffusionRSHVolumeSliceDisplayNode *dnode = this->GlyphDisplayNodes[0];
    dpnode = vtkMRMLDiffusionRSHDisplayPropertiesNode::SafeDownCast( dnode->GetDiffusionRSHDisplayPropertiesNode() );
    }
  return dpnode;
  }
