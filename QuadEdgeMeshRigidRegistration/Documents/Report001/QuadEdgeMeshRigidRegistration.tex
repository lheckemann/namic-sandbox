...
The \code{PointLocator} class takes advantage of the \doxygen{KdTreeGenerator}
and \doxygen{PointSetToListSampleAdaptor} classes provided by the ITK
Statistics framework. The KdTree is constructed from the Moving Mesh during the
initialization process of the interpolator, that is triggered by the
Initialize() method of the Metric itself.
