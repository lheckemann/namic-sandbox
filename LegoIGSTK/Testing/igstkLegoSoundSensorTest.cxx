...
int main( int argc, char * argv [] )
{
  ...
  // Turn on the sound sensor
  nxtUSB.SetSensorSound( port, true );

  for(unsigned int k=0; k<1000; k++)
    {
    const int soundLevel = nxtUSB.GetSoundSensor(port);
    std::cout << soundLevel << std::endl;
    }


  // Turn off the sound sensor
  nxtUSB.SetSensorSound( port, false );
